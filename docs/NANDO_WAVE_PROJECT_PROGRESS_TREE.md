...
|   +-- real traffic routability / savings claim     [NOT STARTED]
|   |   |
|   |   +-- required input                           [real Codex/API/agent traces]
|   |   +-- required mode                            [shadow first, no production replacement claim]
|   |   +-- must report                              [total_llm_calls / exact_cache_hits / nando_routable_calls]
|   |   +-- must report                              [local_accepts / fallbacks / false_local_accepts]
|   |   +-- must report                              [incremental_reduction_vs_exact_cache]
|   |   +-- market claim boundary                    [synthetic 50% is not enough for sales claim]
