...
|   +-- retrieval_lookup payload dry-run             [REVIEW, scoreable subset only]
|   |   |
...
|   |   +-- local_accepts_enabled                     [false]
|   |   +-- market_claim_allowed                      [false]
|   |   +-- boundary                                  [dry-run only; not read_inspect and not CPU savings until source/path verifier exists]
