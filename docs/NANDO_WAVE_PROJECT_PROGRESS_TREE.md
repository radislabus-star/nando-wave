...
|   +-- real traffic routability / savings claim     [REVIEW, current default unique 16/1000]
|   |   |
|   |   +-- source                                  [non-synthetic Codex history trace]
|   |   +-- current default verified CPU route-sum accepts [21 / 1000]
|   |   +-- current default unique verified CPU accepts [16 / 1000]
...
|   |   +-- current default scoreable_candidate_calls [175]
|   |   +-- current default verification_hook_ready_events [136]
|   |   +-- false_accepts                            [0]
|   |   +-- current default unique_gap_to_80_calls   [784]
|   |   +-- current default agent-control route      [v2 visible row: 143 candidates, 11 route-sum verified]
|   |   +-- current default mixed route              [v2 visible row: 39 candidates, 3 route-sum verified]
...
|   |   +-- market claim boundary                    [not achieved; current default unique 16 milli != 800 milli]
...
|   +-- project_context_dialogue payload dry-run     [REVIEW, scoreable subset only]
|   |   |
|   |   +-- dry_run_report                           [project-context-payload-dry-run-v1.report.json]
|   |   +-- dry_run_trace                            [project-context-payload-dry-run-v1.trace.jsonl]
|   |   +-- candidate_events                         [211 / 1000]
...
|   |   +-- boundary                                  [dry-run only; does not change current 16/1000 unique CPU scoreboard]
|   +-- project_context_dialogue subfamily audit     [REVIEW, route split required]
|   |   |
|   |   +-- audit_report                             [project-context-subfamily-audit-v1.report.json]
|   |   +-- candidate_events                         [211 / 1000]
|   |   +-- payload_ready_events                      [2]
...
|   |   +-- local_accepts_enabled                     [false]
|   |   +-- market_claim_allowed                      [false]
|   |   +-- decision                                  [do not inflate project_context; split or fall back until explicit state/action/evidence exists]
|   +-- retrieval_lookup route/profile/evidence rung [REVIEW, hooks ready, accepts disabled]
|   |   |
|   |   +-- dry_run_report                           [retrieval-lookup-payload-dry-run-v1.report.json]
|   |   +-- dry_run_trace                            [retrieval-lookup-payload-dry-run-v1.trace.jsonl]
|   |   +-- profile_report                           [retrieval-lookup-profile-v1.report.json]
|   |   +-- registry                                 [profile-registry-retrieval-lookup-v1.json]
|   |   +-- package                                  [retrieval-lookup-seed0.nwrb]
|   |   +-- output_evidence_report                   [retrieval-lookup-output-evidence-v1.report.json]
|   |   +-- output_evidence_audit                    [retrieval-lookup-output-evidence-v1.verification-hook-audit.report.json]
|   |   +-- shadow_report                            [retrieval-lookup-output-evidence-v1.shadow-report.json]
|   |   +-- local_accept_calibration_report          [retrieval-lookup-local-accept-calibration-v1.report.json]
...
|   |   +-- verifier_true_events                     [2]
|   |   +-- verifier_false_events                    [0]
|   |   +-- verification_hook_ready_events           [2]
|   |   +-- calibration_stage                        [support_insufficient; 2 true rows < 3 minimum support]
|   |   +-- feedback_stage                           [local_accept_calibration_support_insufficient]
|   |   +-- verified_cpu_accepts                     [0]
|   |   +-- shadow_nando_accepts                     [0]
|   |   +-- shadow_verified_safe_accepts             [0]
|   |   +-- shadow_false_accepts                     [0]
|   |   +-- shadow_p99_score_latency_ns              [250185]
|   |   +-- profile_registered                        [true, overlay registry only]
|   |   +-- local_accepts_enabled                     [false]
|   |   +-- market_claim_allowed                      [false]
|   |   +-- boundary                                  [complete disabled route rung now wired into feedback/catalog; still adds 0 verified CPU accepts]
...
|   +-- current default feedback after retrieval_lookup wiring + read_inspect + metrics_report calibration + agent-control v2 safe-policy + mixed v2 safe-policy + git_control safe-policy + serving_ops safe-policy [REVIEW, unique 16/1000]
|   |   |
|   |   +-- feedback_report                          [cpu-route-feedback-loop-v1.report.json]
|   |   +-- operator_candidate_calls                 [595 / 1000]
|   |   +-- no_candidate_calls                       [405 / 1000]
...
|   |   +-- incremental_unique_cpu_accepts           [16]
|   |   +-- duplicate_verified_route_hits            [5]
|   |   +-- exact_cache_overlap_verified_cpu_accepts [1]
|   |   +-- verified_gap_to_80_calls                 [779 route-sum, not scoreboard]
...
|   |   +-- mixed_stage                              [verified_cpu_accept_eligible; v2 39 candidates, 3 route-sum accepts]
|   |   +-- retrieval_lookup_stage                   [support_insufficient; 25 candidates, 2 scoreable, 0 accepts]
...
|   |   |
|   |   +-- catalog_report                           [cpu-operator-catalog-v1.report.json]
|   |   +-- current_verified_cpu_accepts             [16 unique]
|   |   +-- verified_cpu_accept_route_sum_events     [21]
...
|   |   +-- retrieval_lookup_catalog_rank            [15 existing_profile_route; 25 candidates, 2 scoreable, 0 accepts]
