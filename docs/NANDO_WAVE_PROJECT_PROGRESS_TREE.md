...
|   +-- git_control route/profile/evidence rung     [REVIEW, hooks ready, accepts disabled]
|   |   |
...
|   |   +-- verified_cpu_accept_eligible_events      [0]
|   |   +-- false_accepts                            [0]
|   |   +-- market claim boundary                    [not savings; final-answer evidence only; no git command execution; local_accepts_enabled=false]
