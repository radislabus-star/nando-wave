...
|   +-- real traffic routability / savings claim     [REVIEW, current default 8/1000; historical strongest 17/1000]
|   |   |
|   |   +-- source                                  [non-synthetic Codex history trace]
|   |   +-- current default verified CPU accepts     [8 / 1000]
...
|   |   +-- market claim boundary                    [not achieved; current default 8 milli != 800 milli]
...
|   +-- git_control route/profile/evidence/calibration rung [REVIEW, needs stronger verifier]
|   |   |
...
|   |   +-- verified_cpu_accept_eligible_events      [0]
|   |   +-- false_accepts                            [0]
|   |   +-- market claim boundary                    [not savings; final-answer labels only; no real tool-output/status verifier; no git execution]
...
|   +-- current default feedback after read_inspect + metrics_report calibration + git_control calibration + serving_ops safe-policy [REVIEW, 11/1000]
|   |   |
