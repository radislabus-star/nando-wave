...
```text
The old CPU call catalog is archived.
The active catalog should be generated by the phase-center stream:
  real events
  -> opportunity board
  -> marginal denominator value
  -> verifier readiness
  -> selected phase-center candidates
  -> shadow proof
```
