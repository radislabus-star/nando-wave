#!/usr/bin/env python3
"""Shortcut gates for binding-pressure action traces."""
...
TASKS = ROOT / "accepted_binding_pressure_tasks_v1.jsonl"
...
def split_rows(rows: list[dict[str, object]]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    train = [row for row in rows if str(row["source_group"]).startswith("binding_train_")]
    heldout = [row for row in rows if str(row["source_group"]).startswith("binding_heldout_")]
...
def verdict(metrics: dict[str, int]) -> str:
    ...
    if metrics["markov_pairwise_accuracy_milli"] > 650:
        return "REJECT_MARKOV_BINDING_SHORTCUT"
    return "VALID_BINDING_PRESSURE_CANDIDATE"
...
def main() -> int:
    ...
    report = {
        "schema_version": "binding_pressure_shortcut_gate_v1",
        "metrics": metrics,
        "verdict": verdict(metrics),
    }
    REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if report["verdict"] == "VALID_BINDING_PRESSURE_CANDIDATE" else 1
