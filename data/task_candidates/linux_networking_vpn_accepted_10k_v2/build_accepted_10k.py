#!/usr/bin/env python3
"""Build a 10k Wave Task V2 corpus with mixed source groups and hard negatives."""
...
def mixed_source_group(case_index: int, variant_index: int) -> str:
    # Source group is deliberately not the operator. Each bucket receives all
    # operator families, so source_group-only cannot act as answer authority.
    bucket = (variant_index + 3 * case_index) % 16
    return f"vpn_mixed_bucket_{bucket:02d}"


def assign_cross_target_negatives(rows: list[dict], variants_per_case: int) -> None:
    for index, row in enumerate(rows):
        decoy = rows[(index + variants_per_case) % len(rows)]
        row["near_negative"] = decoy["target"]
        row["why_negative_is_wrong"] = (
            "Near-negative является настоящим target-текстом из того же общего VPN-корпуса, "
            f"но принадлежит оператору {decoy['operator_family']}, а текущий evidence требует "
...
def main() -> int:
            ...
            rows.append(
                {
                    ...
                    "task_id": f"lnvpn_accepted_10k_v2_{serial:06d}",
                    "language": case["language"],
                    "task_kind": template["task_kind"],
                    "domain_path": domain["domain_path"],
                    "domain_tags": domain_tags,
                    "source_family": "domain_dsl_accepted_10k_v2",
                    "source_group": mixed_source_group(case_index, variant_index),
                    "input": input_text,
                    "target": target_text,
                    "near_negative": variant_target(case["near_negative"], variant_index),
                    ...
                    "why_negative_is_wrong": (
                        "Near-negative будет заменен после сборки на настоящий target-текст "
                        f"другого оператора; базовое правило: {first_negative['negative_rule_id']}."
                    ),
    ...
    assign_cross_target_negatives(rows, variants_per_case)
