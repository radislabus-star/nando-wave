#!/usr/bin/env python3
"""Strict shortcut gates for Rule Logic v2."""

from __future__ import annotations

import json
import math
import re
from collections import Counter, defaultdict
from pathlib import Path


ROOT = Path(__file__).resolve().parent
TASKS = ROOT / "accepted_rule_tasks_v2.jsonl"
REPORT = ROOT / "shortcut_gate_report.json"
TOKEN_RE = re.compile(r"[A-Za-zА-Яа-я0-9_]+|[{}:;?=<>|,-]")


def tokens(text: str) -> list[str]:
    return [token.lower() for token in TOKEN_RE.findall(text)]


def load_rows() -> list[dict[str, object]]:
    rows = []
    with TASKS.open("r", encoding="utf-8") as handle:
        for line in handle:
            if line.strip():
                rows.append(json.loads(line))
    return rows


def split_rows(rows: list[dict[str, object]]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    train = []
    heldout = []
    for index, row in enumerate(rows):
        (heldout if index % 5 == 0 else train).append(row)
    return train, heldout


def milli(num: int, den: int) -> int:
    return 0 if den == 0 else round(1000 * num / den)


def exact_lookup(train: list[dict[str, object]], heldout: list[dict[str, object]]) -> int:
    table = {str(row["input"]): str(row["target"]) for row in train}
    return milli(sum(1 for row in heldout if table.get(str(row["input"])) == str(row["target"])), len(heldout))


def majority_by_key(train: list[dict[str, object]], heldout: list[dict[str, object]], key: str) -> int:
    groups: dict[str, Counter[str]] = defaultdict(Counter)
    for row in train:
        groups[str(row[key])][str(row["target"])] += 1
    hits = 0
    seen = 0
    for row in heldout:
        group = groups.get(str(row[key]))
        if not group:
            continue
        seen += 1
        hits += int(group.most_common(1)[0][0] == str(row["target"]))
    return milli(hits, seen)


def markov_choice(train: list[dict[str, object]], heldout: list[dict[str, object]]) -> int:
    counts: dict[str, Counter[str]] = defaultdict(Counter)
    vocab = set()
    for row in train:
        input_tokens = set(tokens(str(row["input"])))
        for target_token in tokens(str(row["target"])):
            vocab.add(target_token)
            for input_token in input_tokens:
                counts[input_token][target_token] += 1
    vocab_size = max(1, len(vocab))

    def score(input_text: str, candidate: str) -> float:
        input_tokens = set(tokens(input_text))
        total = 0.0
        for cand_token in tokens(candidate):
            prob = 1.0
            for input_token in input_tokens:
                counter = counts.get(input_token)
                if not counter:
                    continue
                prob += (counter[cand_token] + 1) / (sum(counter.values()) + vocab_size)
            total += math.log(prob)
        return total

    hits = 0
    ties = 0
    for row in heldout:
        target_score = score(str(row["input"]), str(row["target"]))
        negative_score = score(str(row["input"]), str(row["near_negative"]))
        if target_score > negative_score:
            hits += 1
        elif target_score == negative_score:
            ties += 1
    return milli(hits + ties // 2, len(heldout))


def target_label_balance(rows: list[dict[str, object]]) -> dict[str, int]:
    counts = Counter(str(row["target"]) for row in rows)
    return dict(sorted(counts.items()))


def verdict(metrics: dict[str, object]) -> str:
    exact = int(metrics["exact_lookup_accuracy_milli"])
    source = int(metrics["source_group_majority_accuracy_milli"])
    rule = int(metrics["proof_rule_id_majority_accuracy_milli"])
    surface = int(metrics["surface_family_majority_accuracy_milli"])
    status = int(metrics["answer_status_majority_accuracy_milli"])
    markov = int(metrics["markov_choice_accuracy_milli"])
    if exact > 0:
        return "REJECT_EXACT_LOOKUP_SHORTCUT"
    if source > 300:
        return "REJECT_SOURCE_GROUP_SHORTCUT"
    if rule > 300:
        return "REJECT_RULE_ID_SHORTCUT"
    if surface > 300:
        return "REJECT_SURFACE_FAMILY_SHORTCUT"
    if status > 300:
        return "REJECT_STATUS_SHORTCUT"
    if markov > 600:
        return "REJECT_MARKOV_SHORTCUT"
    return "VALID_RULE_OPERATOR_PRESSURE_CANDIDATE_V2"


def main() -> int:
    rows = load_rows()
    train, heldout = split_rows(rows)
    metrics: dict[str, object] = {
        "rows": len(rows),
        "train_rows": len(train),
        "heldout_rows": len(heldout),
        "exact_lookup_accuracy_milli": exact_lookup(train, heldout),
        "source_group_majority_accuracy_milli": majority_by_key(train, heldout, "source_group"),
        "proof_rule_id_majority_accuracy_milli": majority_by_key(train, heldout, "proof_rule_id"),
        "surface_family_majority_accuracy_milli": majority_by_key(train, heldout, "surface_family"),
        "answer_status_majority_accuracy_milli": majority_by_key(train, heldout, "answer_status"),
        "markov_choice_accuracy_milli": markov_choice(train, heldout),
        "target_label_balance": target_label_balance(rows),
    }
    report = {
        "schema_version": "rule_logic_shortcut_gate_report_v2",
        "metrics": metrics,
        "verdict": verdict(metrics),
    }
    REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if report["verdict"] == "VALID_RULE_OPERATOR_PRESSURE_CANDIDATE_V2" else 1


if __name__ == "__main__":
    raise SystemExit(main())
