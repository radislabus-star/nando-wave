#!/usr/bin/env python3
"""Fail-closed verifier for the frozen S1C-3 V2 transaction receipt."""
...
SCHEMA = "nando.s1c3-transaction-receipt.v2"
...
def verify_quiescence(path: Path) -> tuple[dict[str, Any], dict[str, Any], str]:
...
def verify_receipt(directory: Path) -> dict[str, Any]:
    quiescence, executables, executable_root = verify_quiescence(
        directory / "quiescence-receipt.json"
    )
    ...
    return {
        "schema": "nando.s1c3-verification.v2",
        "valid": True,
        "verdict": verdict,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "nando.s1c3-verification.v2", "valid": False,
