...
[[ $(sha256sum plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V7/SHA256SUMS | awk '{print $1}') == "$paper_manifest_root" ]] || {
...
transaction_id="${timestamp}-${paper_commit:0:12}-s1c3v7"
...
prior_attempts=$(ssh "$remote" \
  "find /var/lib/nando-wave/deployments -mindepth 1 -maxdepth 1 \
    -type d -name '*-${paper_commit:0:12}-s1c3v7' -print | wc -l")
[[ $prior_attempts == 0 ]] || {
  printf 'v7_attempt_already_exists count=%s\n' "$prior_attempts" >&2
  exit 2
}
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v7.py' prepare \
...
if [[ $prepare_code -ne 0 ]]; then
  ...
  if [[ $state == QUIESCENCE_TIMEOUT ]]; then
    ...
    set +e
    python3 ops/remote-backend/verify_s1c3_transaction_v7.py "$local_dir" \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/verify_s1c3_transaction_v7.py' \
...
emergency_rollback() {
  ...
  if [[ $rollback_armed == true ]]; then
    ssh "$remote" \
      "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v7.py' rollback \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v7.py' execute \
    --transaction-directory '$remote_transaction' \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v7.py' finalize \
    --transaction-directory '$remote_transaction' \
...
set +e
python3 ops/remote-backend/verify_s1c3_transaction_v7.py "$local_dir" --allow-rollback \
  > "$local_dir/local-verification.json"
verify_code=$?
set -e
