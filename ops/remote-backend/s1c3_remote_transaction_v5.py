#!/usr/bin/env python3
"""Root-only remote executor for the preregistered S1C-3 V5 transaction."""
...
RECEIPT_SCHEMA = "nando.s1c3-transaction-receipt.v5"
STATE_SCHEMA = "nando.s1c3-state.v5"
PENDING_SCHEMA = "nando.s1c3-pending-receipt.v5"
...
class MeasurementMonitor:
    def __init__(
        ...
        self.thread = threading.Thread(target=self._loop, name="s1c3-v5-monitor", daemon=True)
...
def run_oracle_ownership_probe(
    ...
    probe_code = (
        "import json,os,sys;d=sys.argv[1];p=os.path.join(d,'.s1c3-v5-ownership-probe');"
        "fd=os.open(p,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);"
        "os.write(fd,b's1c3-v5\\n');os.fsync(fd);os.close(fd);s=os.stat(p);os.unlink(p);"
    ...
    probe_path = workspace / ".s1c3-v5-ownership-probe"
...
def make_oracle(
    ...
    write_json(evidence / f"oracle-ownership-{label}.json", row)
    return cargo_toml, row
...
def prebuild_oracle(
    ...
    manifest, ownership = make_oracle(oracle_source, source, work, label, evidence)
...
def prepare(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        baseline_oracle, baseline_ownership = prebuild_oracle(
            oracle_source, baseline, work, "baseline", root / "evidence"
        )
        candidate_oracle, candidate_ownership = prebuild_oracle(
            oracle_source, source, work, "candidate", root / "evidence"
        )
        ...
        if measurement_error is not None:
            write_json(
                ...
                {
                    "schema": "nando.s1c3-measurement-failure.v5",
                    "observed_at": utc_now(),
        ...
        write_json(root / "preflight-failure.json", {"schema": "nando.s1c3-preflight-failure.v5",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "nando.s1c3-executor-error.v5", "error": str(error)}, sort_keys=True), file=sys.stderr)
        return 2
