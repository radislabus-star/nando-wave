#!/usr/bin/env python3
"""Root-only remote executor for the preregistered S1C-3 V3 transaction."""
...
PAPER_VERIFICATION_SHA256 = "e4d3c5546f1807555de1e76fe2fb3b6dfc5d222a91859659fce88a2f2fda79fa"
...
OWNERSHIP_SCHEMA = "nando.s1c3-oracle-ownership-receipt.v3"
OWNERSHIP_ROW_SCHEMA = "nando.s1c3-oracle-ownership-row.v3"
QUIESCENCE_SCHEMA = "nando.s1c3-quiescence-receipt.v3"
CONTAMINATION_SCHEMA = "nando.s1c3-measurement-contamination-receipt.v3"
RESOURCE_SCHEMA = "nando.s1c3-resource-receipt.v3"
PARITY_SCHEMA = "nando.s1c3-parity-receipt.v3"
PREPARATION_SCHEMA = "nando.s1c3-transaction-preparation.v3"
RECEIPT_SCHEMA = "nando.s1c3-transaction-receipt.v3"
STATE_SCHEMA = "nando.s1c3-state.v3"
PENDING_SCHEMA = "nando.s1c3-pending-receipt.v3"
...
class MeasurementMonitor:
    def __init__(self, transaction_id: str, quiescence_root: str, executable_root: str) -> None:
        ...
        self.thread = threading.Thread(target=self._loop, name="s1c3-v3-monitor", daemon=True)
...
def run_oracle_ownership_probe(
    ...
    probe_code = (
        "import json,os,sys;d=sys.argv[1];p=os.path.join(d,'.s1c3-v3-ownership-probe');"
        "fd=os.open(p,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);"
        "os.write(fd,b's1c3-v3\\n');os.fsync(fd);os.close(fd);s=os.stat(p);os.unlink(p);"
    ...
    probe_path = workspace / ".s1c3-v3-ownership-probe"
    ...
    try:
        ...
        if measurement_error is not None:
            write_json(
                ...
                {
                    "schema": "nando.s1c3-measurement-failure.v3",
                    "observed_at": utc_now(),
        ...
        write_json(root / "preflight-failure.json", {"schema": "nando.s1c3-preflight-failure.v3",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "nando.s1c3-executor-error.v3", "error": str(error)}, sort_keys=True), file=sys.stderr)
        return 2
