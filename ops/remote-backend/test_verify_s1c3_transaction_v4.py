...
MODULE_PATH = Path(__file__).with_name("verify_s1c3_transaction_v4.py")
SPEC = importlib.util.spec_from_file_location("s1c3_verifier", MODULE_PATH)
assert SPEC and SPEC.loader
verifier = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(verifier)

EXECUTOR_PATH = Path(__file__).with_name("s1c3_remote_transaction_v4.py")
...
class TransactionVerifierTests(unittest.TestCase):
    ...
    def test_non_root_probe_smoke_uses_production_helper(self) -> None:
        ...
        self.assertFalse((workspace / ".s1c3-v4-ownership-probe").exists())
    ...
    def test_remote_executor_has_no_build_command_after_quiescence(self) -> None:
        executor = Path(__file__).with_name("s1c3_remote_transaction_v4.py").read_text(
            encoding="utf-8"
        )
