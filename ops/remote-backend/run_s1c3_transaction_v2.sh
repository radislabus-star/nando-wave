...
candidate_config=plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V2/transition-serving.env.candidate
...
[[ $(sha256sum plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V2/SHA256SUMS | awk '{print $1}') == "$paper_manifest_root" ]] || {
...
transaction_id="${timestamp}-${paper_commit:0:12}-s1c3v2"
...
scp -q \
  "$work/source.bundle" \
  ops/remote-backend/s1c3_remote_transaction_v2.py \
  ops/remote-backend/verify_s1c3_transaction_v2.py \
  "$candidate_config" \
  "$parity_source" \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v2.py' prepare \
...
emergency_rollback() {
  ...
  if [[ $rollback_armed == true ]]; then
    ssh "$remote" \
      "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v2.py' rollback \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v2.py' execute \
    --transaction-directory '$remote_transaction'" \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v2.py' finalize \
    --transaction-directory '$remote_transaction' \
...
for name in quiescence-receipt.json measurement-contamination-receipt.json preparation.json resource-receipt.json parity-receipt.json deployment-receipt.json transaction-state.json; do
  ssh "$remote" "sudo -n cat '$remote_transaction/$name'" > "$local_dir/$name"
  if [[ $name == quiescence-receipt.json ]]; then
...
set +e
python3 ops/remote-backend/verify_s1c3_transaction_v2.py "$local_dir" --allow-rollback \
