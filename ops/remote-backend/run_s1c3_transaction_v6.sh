...
candidate_config=plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V6/transition-serving.env.candidate
parity_source=ops/remote-backend/s1c3-parity-oracle/main.rs
oracle_lock=plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V6/oracle.Cargo.lock
...
[[ $(sha256sum plans/effect-law-unification-v1/evidence/S1C_TRANSACTIONAL_DEPLOYMENT_PREREGISTRATION_V6/SHA256SUMS | awk '{print $1}') == "$paper_manifest_root" ]] || {
...
transaction_id="${timestamp}-${paper_commit:0:12}-s1c3v6"
...
prior_attempts=$(ssh "$remote" \
  "find /var/lib/nando-wave/deployments -mindepth 1 -maxdepth 1 \
    -type d -name '*-${paper_commit:0:12}-s1c3v6' -print | wc -l")
[[ $prior_attempts == 0 ]] || {
  printf 'v6_attempt_already_exists count=%s\n' "$prior_attempts" >&2
  exit 2
}
...
scp -q \
  "$work/source.bundle" \
  ops/remote-backend/s1c3_remote_transaction_v6.py \
  ops/remote-backend/verify_s1c3_transaction_v6.py \
  "$candidate_config" \
  "$parity_source" \
  "$oracle_lock" \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v6.py' prepare \
...
if [[ $prepare_code -ne 0 ]]; then
  ...
  state=UNKNOWN
  if [[ -f $local_dir/transaction-state.json ]]; then
    state=$(jq -r .state "$local_dir/transaction-state.json")
  fi
  if [[ $state == QUIESCENCE_TIMEOUT ]]; then
    chmod 0400 \
      "$local_dir/oracle-ownership-receipt.json" \
      "$local_dir/quiescence-receipt.json"
    set +e
    python3 ops/remote-backend/verify_s1c3_transaction_v6.py "$local_dir" \
      > "$local_dir/local-verification.json"
    verify_code=$?
    set -e
    printf 'transaction_directory=%s\n' "$remote_transaction"
    printf 'local_evidence=%s\n' "$local_dir"
    printf 'prepare_code=%s state=%s verify_code=%s production_mutation=no\n' \
      "$prepare_code" "$state" "$verify_code"
    [[ $verify_code -eq 0 ]] || exit 2
    exit 3
  fi
  printf 'transaction_directory=%s\n' "$remote_transaction"
  printf 'local_evidence=%s\n' "$local_dir"
...
jq -e '
  .valid == true and
  .authority == true and
  .verdict == "S1C3_PREPARATION_PASS" and
  (.predeployment_verification_root_sha256 | type == "string" and length == 64)
...
emergency_rollback() {
  ...
  if [[ $rollback_armed == true ]]; then
    ssh "$remote" \
      "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v6.py' rollback \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v6.py' execute \
    --transaction-directory '$remote_transaction' \
...
set +e
ssh "$remote" \
  "sudo -n python3 '$remote_upload/s1c3_remote_transaction_v6.py' finalize \
    --transaction-directory '$remote_transaction' \
...
for name in oracle-ownership-receipt.json quiescence-receipt.json measurement-contamination-receipt.json preparation.json resource-receipt.json parity-receipt.json predeployment-verification.json deployment-receipt.json transaction-state.json; do
...
set +e
python3 ops/remote-backend/verify_s1c3_transaction_v6.py "$local_dir" --allow-rollback \
  > "$local_dir/local-verification.json"
verify_code=$?
set -e
