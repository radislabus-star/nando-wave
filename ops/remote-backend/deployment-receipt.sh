...
UNITS="${NANDO_DEPLOY_RECEIPT_UNITS-nando-transition-serving.service nando-response-learning.service nando-operator-certification-authority.service nando-operator-cleanup-verifier@.service nando-gateway-control.service nando-transport-gateway.service nando-live-transition-gate.service nando-live-transition-gate.path nando-response-admission.service nando-response-admission.path nando-response-admission.timer}"
STATE_ROOTS="${NANDO_DEPLOY_RECEIPT_STATE_ROOTS-/var/lib/nando-wave/transition/multi-source-live-v2}"
CONFIG_PATHS="${NANDO_DEPLOY_RECEIPT_CONFIGS-/etc/nando-gateway/nginx.conf /etc/nando-wave/roles/response-learning.env /opt/nando-wave/ops/phase-center-test-server/bin/nando-live-transition-gate}"
