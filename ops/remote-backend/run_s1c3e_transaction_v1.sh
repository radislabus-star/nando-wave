...
implementation_files=(
  ...
  ops/remote-backend/verify_s1c3e_transaction_v1.py
  ops/remote-backend/test_s1c3e_transaction_v1.py
  ops/remote-backend/run_s1c3e_transaction_v1.sh
)
dependency_files=(
...
# Every local gate precedes timestamp creation and remote side effects.
PYTHONPATH=ops/remote-backend python3 -m unittest \
  ops/remote-backend/test_s1c3e_transaction_v1.py >/dev/null
...
transaction_id="${timestamp}-${head:0:12}-s1c3e-v1"
...
connector_snapshot() {
  ...
  jq -nS --arg schema nando.s1c3e-connector-snapshot.v1 --arg label "$label" \
...
finish_rollback() {
  ...
  ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' finalize \
...
emergency() {
  ...
  if [[ $rollback_armed == true ]]; then
    ...
    if [[ $state == PREPARED ]]; then
      ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' abort-predeployment \
    ...
    if [[ $state == ROLLBACK_ARMED || $state == FINALIZE_PENDING || $state == FINAL_VERIFICATION_PENDING ]]; then
      ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' rollback \
...
prior=$(ssh "$remote" "sudo -n find /var/lib/nando-wave/deployments -mindepth 1 -maxdepth 1 -type d -name '*-${head:0:12}-s1c3e-v1' -print | wc -l")
[[ $prior == 0 ]] || { printf 's1c3e_identity_consumed=%s\n' "$prior" >&2; exit 2; }

connector_snapshot before "$work/connector-before.json"
PYTHONPATH=ops/remote-backend python3 ops/remote-backend/verify_s1c3e_transaction_v1.py create-freeze \
...
ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' prepare \
  ...
  > "$local_dir/prepare-result.json"
rollback_armed=true
trap emergency EXIT INT TERM HUP
mirror_remote

PYTHONPATH=ops/remote-backend python3 ops/remote-backend/verify_s1c3e_transaction_v1.py verify \
  "$local_dir/remote-mirror" \
  --implementation-freeze "$local_dir/remote-mirror/implementation-freeze.json" \
  --predeployment > "$local_dir/s1c3e-predeployment.local.json"
scp -q "$local_dir/s1c3e-predeployment.local.json" "$remote:$remote_upload/predeployment-verification.json"
ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/verify_s1c3e_transaction_v1.py' verify \
  '$remote_transaction' --implementation-freeze '$remote_transaction/implementation-freeze.json' --predeployment" \
  > "$local_dir/s1c3e-predeployment.remote.json"
cmp "$local_dir/s1c3e-predeployment.local.json" "$local_dir/s1c3e-predeployment.remote.json"

set +e
ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' execute \
...
if [[ $state == PREPARED ]]; then
  ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' abort-predeployment \
    --transaction-directory '$remote_transaction' --reason execute_failed_before_mutation" \
    > "$local_dir/preflight-abort.json"
  rollback_armed=false
  trap cleanup EXIT
  printf 'verdict=S1C3E_PREFLIGHT_FAILURE execute_code=%s production_mutation=no\n' "$execute_code"
  exit 3
fi
...
PYTHONPATH=ops/remote-backend python3 ops/remote-backend/verify_s1c3e_transaction_v1.py verify \
  ...
  > "$local_dir/s1c3e-final.local.json"
scp -q "$local_dir/s1c3e-final.local.json" "$remote:$remote_upload/final-verification.json"
ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/verify_s1c3e_transaction_v1.py' verify \
  '$remote_transaction' --implementation-freeze '$remote_transaction/implementation-freeze.json'" \
  > "$local_dir/s1c3e-final.remote.json"
cmp "$local_dir/s1c3e-final.local.json" "$local_dir/s1c3e-final.remote.json"
ssh "$remote" "sudo -n env PYTHONPATH='$remote_upload' python3 '$remote_upload/s1c3e_remote_transaction_v1.py' seal \
...
trap cleanup EXIT
mirror_remote
verdict=$(jq -er .verdict "$local_dir/remote-mirror/s1c3e-state.json")
