#!/usr/bin/env python3
"""Root-only remote executor for the preregistered S1C-3 V4 transaction."""
...
RECEIPT_SCHEMA = "nando.s1c3-transaction-receipt.v4"
STATE_SCHEMA = "nando.s1c3-state.v4"
PENDING_SCHEMA = "nando.s1c3-pending-receipt.v4"
...
class MeasurementMonitor:
    def __init__(self, transaction_id: str, quiescence_root: str, executable_root: str) -> None:
        ...
        self.thread = threading.Thread(target=self._loop, name="s1c3-v4-monitor", daemon=True)
...
def run_oracle_ownership_probe(
    ...
    probe_code = (
        "import json,os,sys;d=sys.argv[1];p=os.path.join(d,'.s1c3-v4-ownership-probe');"
        "fd=os.open(p,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);"
        "os.write(fd,b's1c3-v4\\n');os.fsync(fd);os.close(fd);s=os.stat(p);os.unlink(p);"
    ...
    probe_path = workspace / ".s1c3-v4-ownership-probe"
    ...
    try:
        ...
        if measurement_error is not None:
            write_json(
                ...
                {
                    "schema": "nando.s1c3-measurement-failure.v4",
                    "observed_at": utc_now(),
        ...
        write_json(root / "preflight-failure.json", {"schema": "nando.s1c3-preflight-failure.v4",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "nando.s1c3-executor-error.v4", "error": str(error)}, sort_keys=True), file=sys.stderr)
        return 2
