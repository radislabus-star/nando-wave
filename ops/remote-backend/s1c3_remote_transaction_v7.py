#!/usr/bin/env python3
"""Root-only remote executor for the preregistered S1C-3 V7 transaction."""
...
PRODUCTION_PROJECTION_PATH = Path(
...
STAGE_SYNC_RE = re.compile(
    r"S1C3_STAGE_SYNC_LATENCY precommit_p99_ns=(\d+) precommit_hard_max_ns=(\d+) "
    r"settlement_p99_ns=(\d+) settlement_hard_max_ns=(\d+) episode_p99_ns=(\d+) "
    r"episode_hard_max_ns=(\d+) records=(\d+)"
)
IDLE_RE = re.compile(
...
RECEIPT_SCHEMA = "nando.s1c3-transaction-receipt.v7"
STATE_SCHEMA = "nando.s1c3-state.v7"
PENDING_SCHEMA = "nando.s1c3-pending-receipt.v7"
...
LOGICAL_CPU_POOL = tuple(
    cpu for representative in MEASUREMENT_CPU_POOL
...
def production_projection_sha256(path: Path) -> str:
    marker = b"#[cfg(test)]\n"
    payload = path.read_bytes()
    if payload.count(marker) != 1:
        raise GateFailure("candidate_production_projection_boundary_invalid")
    boundary = payload.index(marker) + len(marker)
    return sha256_bytes(payload[:boundary])
...
def isolate_observer_cpus() -> None:
    ...
    observer = available - set(LOGICAL_CPU_POOL)
    if not observer:
...
def observation_error(error: BaseException) -> dict[str, str]:
    if isinstance(error, (FileNotFoundError, ProcessLookupError)):
        return {"status": "ENOENT"}
    if isinstance(error, PermissionError):
        return {"status": "PERMISSION_DENIED"}
    if isinstance(error, UnicodeError):
        return {"status": "MALFORMED", "error": type(error).__name__}
    if isinstance(error, OSError):
        return {"status": "OS_ERROR", "error": str(error.errno)}
    return {"status": "MALFORMED", "error": type(error).__name__}


def read_stat_observation(path: Path) -> dict[str, Any]:
    try:
...
def read_status_observation(path: Path) -> dict[str, Any]:
    try:
        lines = path.read_text(encoding="utf-8").splitlines()
    except (OSError, UnicodeError) as error:
        return observation_error(error)
    matches = [line for line in lines if line.startswith("Kthread:")]
    if len(matches) != 1:
        return {"status": "MALFORMED", "error": "kthread_field_invalid"}
    return {"status": "VALUE", "kthread_line": matches[0]}


def read_cmdline_observation(path: Path) -> dict[str, Any]:
    try:
        payload = path.read_bytes()
...
def read_exe_observation(path: Path) -> dict[str, Any]:
    try:
...
def classify_process_observation(observation: dict[str, Any]) -> dict[str, Any]:
    pid = observation["pid"]
    opening_observation = observation["opening_stat"]
    closing_observation = observation["closing_stat"]
    status_observation = observation["status"]
    cmdline_observation = observation["cmdline"]
    exe_observation = observation["exe"]
    ...
    try:
        ...
        if set(opening) != {"pid", "comm", "state", "starttime"}:
            raise ValueError("opening_stat_fields")
    except (KeyError, TypeError, ValueError):
        result["reason"] = "opening_stat_malformed"
        return result
    if opening["pid"] != pid:
        result["reason"] = "opening_stat_pid_mismatch"
        return result

    if closing_observation.get("status") == "ENOENT":
        result.update({
        ...
        return result
    if closing_observation.get("status") != "VALUE":
        result["reason"] = f"closing_stat_{closing_observation.get('status', 'MISSING').lower()}"
        return result
    try:
        closing = closing_observation["parsed"]
        if set(closing) != {"pid", "comm", "state", "starttime"}:
            raise ValueError("closing_stat_fields")
    except (KeyError, TypeError, ValueError):
        result["reason"] = "closing_stat_malformed"
        return result
    if closing["pid"] != pid:
        result["reason"] = "closing_stat_pid_mismatch"
        return result
    if opening["starttime"] != closing["starttime"]:
    ...
    if opening["state"] == closing["state"] == "Z":
        ...
        return result
    auxiliary = (status_observation, cmdline_observation, exe_observation)
    if any(row.get("status") not in {"VALUE", "ENOENT"} for row in auxiliary):
        bad = next(row for row in auxiliary if row.get("status") not in {"VALUE", "ENOENT"})
        result["reason"] = f"auxiliary_{bad.get('status', 'MISSING').lower()}"
        return result
    if status_observation.get("status") != "VALUE":
        result["reason"] = f"status_{status_observation.get('status', 'MISSING').lower()}"
        return result
    if cmdline_observation.get("status") != "VALUE":
        result["reason"] = f"cmdline_{cmdline_observation.get('status', 'MISSING').lower()}"
        return result
    try:
    ...
    if exe_observation.get("status") == "VALUE":
        basename = exe_observation.get("basename")
        target = exe_observation.get("target")
        if not isinstance(target, str) or not isinstance(basename, str) or basename != os.path.basename(target):
            result["reason"] = "executable_shape_invalid"
            return result
        forbidden = sorted(
            {opening["comm"], closing["comm"], basename}.intersection(FORBIDDEN_BUILD_NAMES)
        )
        result.update({
...
def process_row_projection(row: dict[str, Any]) -> dict[str, Any]:
    identity = row.get("identity") or {}
    return {
        "pid": row["pid"],
        "reason": row["reason"],
        "starttime": identity.get("starttime"),
        "opening_comm": identity.get("opening_comm", identity.get("comm")),
...
def build_process_snapshot() -> dict[str, Any]:
    ...
    for entry in sorted(Path("/proc").iterdir(), key=lambda path: int(path.name) if path.name.isdigit() else -1):
        ...
        observation = {
            "pid": int(entry.name),
            "opening_stat": read_stat_observation(entry / "stat"),
            "status": read_status_observation(entry / "status"),
            "cmdline": read_cmdline_observation(entry / "cmdline"),
            "exe": read_exe_observation(entry / "exe"),
            "closing_stat": read_stat_observation(entry / "stat"),
        }
        rows.append({**observation, **classify_process_observation(observation)})
    reason_counts = {
    ...
    summary = {
        ...
        "observable_user_process_count": sum(
            row["classification"] == OBSERVABLE_USER_PROCESS for row in rows
        ),
        "proven_vanished_count": sum(row["reason"] == "PID_VANISHED" for row in rows),
        "proven_zombie_count": sum(row["reason"] == "STABLE_ZOMBIE" for row in rows),
        "proven_kernel_thread_count": sum(
            row["reason"] == "STABLE_KERNEL_THREAD" for row in rows
        ),
        "unresolved_process_count": sum(
            row["classification"] == UNRESOLVED_PROCESS_OBSERVATION for row in rows
        ),
        "reason_counts": reason_counts,
    }
    unresolved = [
        process_row_projection(row) for row in rows
        if row["classification"] == UNRESOLVED_PROCESS_OBSERVATION
    ]
    forbidden = [
        {**process_row_projection(row), "matched_names": row["forbidden_names"]}
        for row in rows if row["forbidden_names"]
    ]
    return add_root({
        "schema": PROCESS_SNAPSHOT_SCHEMA,
        "detector_schema": PROCESS_DETECTOR_SCHEMA,
        "rows": rows,
        "summary": summary,
...
def cpu_counters(cpus: tuple[int, ...] = LOGICAL_CPU_POOL) -> dict[str, dict[str, int]]:
    wanted = {f"cpu{cpu}": cpu for cpu in cpus}
    counters: dict[str, dict[str, int]] = {}
    payload = Path("/proc/stat").read_text(encoding="ascii")
    for line in payload.splitlines():
        ...
        counters[str(wanted[label])] = {"total": total, "idle": idle}
    if set(counters) != {str(cpu) for cpu in cpus}:
        raise GateFailure(f"cpu_stat_pool_missing:{sorted(counters)}")
    return counters
...
def cpu_topology() -> list[dict[str, Any]]:
    ...
    for representative in MEASUREMENT_CPU_POOL:
        ...
        for cpu in siblings:
            ...
            online = root.exists() and (
                not online_path.exists()
                or online_path.read_text(encoding="ascii").strip() == "1"
            )
            core_id = int((root / "topology/core_id").read_text(encoding="ascii"))
            max_frequency = int(
                (root / "cpufreq/cpuinfo_max_freq").read_text(encoding="ascii")
            )
            logical_rows.append({
                "cpu": cpu,
                "online": online,
        ...
        expected_core_id = EXPECTED_CORE_IDS[representative]
        if any(not row["online"] for row in logical_rows):
            raise GateFailure(f"measurement_cpu_offline:{representative}")
        if {row["core_id"] for row in logical_rows} != {expected_core_id}:
            raise GateFailure(f"measurement_core_topology_mismatch:{representative}")
        if {
            row["max_frequency_khz"] for row in logical_rows
        } != {EXPECTED_MAX_FREQUENCY_KHZ}:
            raise GateFailure(f"measurement_frequency_class_mismatch:{representative}")
        rows.append({
            "representative_cpu": representative,
            "siblings": list(siblings),
            "core_id": expected_core_id,
            "max_frequency_khz": EXPECTED_MAX_FREQUENCY_KHZ,
            "logical_cpus": logical_rows,
        })
    if rows[0]["core_id"] == rows[1]["core_id"]:
        raise GateFailure("measurement_physical_cores_not_distinct")
    return rows
...
def host_observation(label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "build_processes": processes["forbidden_process_matches"],
...
def interval_sample(start: dict[str, Any], end: dict[str, Any]) -> dict[str, Any]:
    ...
    busy_by_cpu: dict[str, float] = {}
    for cpu in LOGICAL_CPU_POOL:
        key = str(cpu)
        total_delta = end["cpu_counters"][key]["total"] - start["cpu_counters"][key]["total"]
        idle_delta = end["cpu_counters"][key]["idle"] - start["cpu_counters"][key]["idle"]
        if total_delta <= 0 or idle_delta < 0 or idle_delta > total_delta:
            raise GateFailure(f"cpu_counter_delta_invalid:{cpu}")
        busy_by_cpu[key] = 100.0 * (total_delta - idle_delta) / total_delta
    some = float(end["io_pressure"]["some"]["avg10"])
    full = float(end["io_pressure"]["full"]["avg10"])
    blockers = []
    if start["build_processes"] or end["build_processes"]:
        blockers.append("forbidden_build_process")
    if start["unresolved_processes"] or end["unresolved_processes"]:
        blockers.append("unresolved_process_observation")
    if not QUIESCENCE_INTERVAL_MIN_SECONDS <= elapsed <= QUIESCENCE_INTERVAL_MAX_SECONDS:
        blockers.append("interval_duration")
    for cpu in LOGICAL_CPU_POOL:
        if busy_by_cpu[str(cpu)] > QUIESCENCE_CPU_MAX_PERCENT:
            blockers.append(f"cpu_busy_per_interval:{cpu}")
    if some > QUIESCENCE_IO_SOME_AVG10_MAX:
        blockers.append("io_some")
    if full > QUIESCENCE_IO_FULL_AVG10_MAX:
        blockers.append("io_full")
    global_blockers = {
    ...
    eligible_cores = [
        representative
        for representative, siblings in PHYSICAL_CORE_SIBLINGS.items()
        if not global_blockers.intersection(blockers)
        and all(f"cpu_busy_per_interval:{cpu}" not in blockers for cpu in siblings)
    ]
    return {
        ...
        "start_cpu_counters": start["cpu_counters"],
        "end_cpu_counters": end["cpu_counters"],
        "start_cpu_snapshot_root_sha256": start["cpu_snapshot_root_sha256"],
        "end_cpu_snapshot_root_sha256": end["cpu_snapshot_root_sha256"],
        "cpu_busy_percent": busy_by_cpu,
        "io_some_avg10": some,
        "io_full_avg10": full,
        "build_processes_start": start["build_processes"],
        "build_processes_end": end["build_processes"],
        "start_process_snapshot_root_sha256": start["process_snapshot_root_sha256"],
        "end_process_snapshot_root_sha256": end["process_snapshot_root_sha256"],
        "loadavg_end": end["loadavg"],
        "blockers": blockers,
        "eligible_physical_cores": eligible_cores,
    }


def blocker_census(samples: list[dict[str, Any]]) -> dict[str, int]:
    keys = [
        "forbidden_build_process",
        "unresolved_process_observation",
        "interval_duration",
    ...
    census = {key: 0 for key in keys}
    for sample in samples:
        for blocker in sample["blockers"]:
            census[blocker] += 1
        for cpu in sample.get("window_mean_blockers", []):
...
def wait_for_quiescence(
    ...
    process_snapshots = [previous["process_snapshot"]]
    while time.monotonic() - started < QUIESCENCE_MAX_WAIT_SECONDS:
        ...
        process_snapshots.append(current["process_snapshot"])
        ...
        for representative in MEASUREMENT_CPU_POOL:
            if representative not in sample["eligible_physical_cores"]:
            ...
            if len(windows[representative]) > QUIESCENCE_REQUIRED_INTERVALS:
                windows[representative].pop(0)
            if len(windows[representative]) < QUIESCENCE_REQUIRED_INTERVALS:
                continue
            means = {
                cpu: sum(row["cpu_busy_percent"][str(cpu)] for row in windows[representative])
                / QUIESCENCE_REQUIRED_INTERVALS
                for cpu in PHYSICAL_CORE_SIBLINGS[representative]
            }
            for cpu, mean in means.items():
                ...
                if mean > QUIESCENCE_CPU_MEAN_MAX_PERCENT:
                    window_mean_blockers.append(cpu)
            if all(mean <= QUIESCENCE_CPU_MEAN_MAX_PERCENT for mean in means.values()):
                passing_cores.append(representative)
        sample["window_mean_blockers"] = sorted(set(window_mean_blockers))
        if passing_cores:
            selected_cpu = min(passing_cores)
            selected_window = list(windows[selected_cpu])
            selected_means = {
                str(cpu): sum(row["cpu_busy_percent"][str(cpu)] for row in selected_window)
                / QUIESCENCE_REQUIRED_INTERVALS
                for cpu in PHYSICAL_CORE_SIBLINGS[selected_cpu]
            }
            verdict = "PASS"
            reached_at: str | None = utc_now()
            break
    else:
        selected_cpu = None
    ...
    attempted_root = sha256_bytes(canonical_bytes(attempted))
    window_root = (
        sha256_bytes(canonical_bytes(selected_window))
        if selected_window is not None else None
    )
    receipt = {
        ...
        "verdict": verdict,
        "measurement_cpu_pool": list(MEASUREMENT_CPU_POOL),
        "selected_cpu": selected_cpu,
        "topology": topology,
        "topology_root_sha256": sha256_bytes(canonical_bytes(topology)),
        "detector_schema": PROCESS_DETECTOR_SCHEMA,
        ...
        "thresholds": {
            ...
            "cpu_per_interval_max_percent": QUIESCENCE_CPU_MAX_PERCENT,
            "cpu_window_mean_max_percent": QUIESCENCE_CPU_MEAN_MAX_PERCENT,
        ...
        "eligibility_reached_at": reached_at,
        "eligibility_started_monotonic_ns": started_monotonic_ns,
        "eligibility_finished_monotonic_ns": finished_monotonic_ns,
        "attempted_interval_count": len(attempted),
        "process_snapshot_count": len(process_snapshots),
        "process_snapshots": process_snapshots,
        "process_snapshots_root_sha256": sha256_bytes(canonical_bytes(process_snapshots)),
        "attempted_samples": attempted,
        "attempted_samples_root_sha256": attempted_root,
        "eligible_window": selected_window,
        "eligible_window_cpu_mean_percent": selected_means,
        "eligible_window_root_sha256": window_root,
        "longest_eligible_streaks": {
            str(cpu): longest_streaks[cpu] for cpu in MEASUREMENT_CPU_POOL
        },
        "minimum_completed_window_mean_percent": {
            str(representative): {
                str(cpu): minimum_means[representative][cpu]
                for cpu in PHYSICAL_CORE_SIBLINGS[representative]
            }
            for representative in MEASUREMENT_CPU_POOL
        },
        "blocker_census": blocker_census(attempted),
        "executables": executables,
        "executable_set_root_sha256": sha256_bytes(canonical_bytes(executables)),
...
class MeasurementMonitor:
    def __init__(
        ...
        self.thread = threading.Thread(target=self._loop, name="s1c3-v7-monitor", daemon=True)
    ...
    def finish(self) -> dict[str, Any]:
        ...
        unresolved = [
            {"sample_index": index, "process": process}
            for index, row in enumerate(ordered)
            for process in row["unresolved_processes"]
        ]
        ...
        receipt = {
            ...
            "measurement_cpu": self.selected_cpu,
            ...
            "unresolved_process_observations": unresolved,
            "monitor_errors": self.errors,
            "contaminated": bool(
                matches or unresolved or self.errors
                or max(gaps, default=0.0) > MONITOR_MAX_GAP_SECONDS
            ),
        }
...
def run_test_binary(
    ...
    try:
        completed = run(
            [
                "taskset", "-c", str(selected_cpu), str(executable), test, "--ignored", "--exact",
...
def file_identity(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "uid": stat.st_uid,
        "gid": stat.st_gid,
        "mode_octal": f"{stat.st_mode & 0o7777:04o}",
    }


def oracle_manifest(source: Path) -> bytes:
    return (
...
def oracle_lock_identity(path: Path, before: str, after: str) -> dict[str, Any]:
    identity = file_identity(path)
    identity.update({
        "sha256_before_build": before,
        "sha256_after_build": after,
    })
    return identity


def offline_dependency_closure_missing(output: bytes) -> bool:
    text = output.decode("utf-8", "replace").lower()
    markers = (
        "attempting to make an http request, but --offline was specified",
        "failed to download",
        "no matching package named",
        "is required, but --offline was specified",
...
def preflight_terminal_state(error: Exception) -> str:
    if str(error).startswith("OFFLINE_DEPENDENCY_CLOSURE_MISSING:"):
        return "OFFLINE_DEPENDENCY_CLOSURE_MISSING"
    return "PREFLIGHT_FAILURE"


def run_oracle_ownership_probe(
    workspace: Path,
    account_name: str,
    log: Path,
) -> dict[str, Any]:
    account = pwd.getpwnam(account_name)
    if account.pw_uid != os.geteuid() and account_name != "e":
        raise GateFailure(f"oracle_ownership_probe_user_unsupported:{account_name}")
    probe_code = (
        "import json,os,sys;d=sys.argv[1];p=os.path.join(d,'.s1c3-v7-ownership-probe');"
        "fd=os.open(p,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);"
        "os.write(fd,b's1c3-v7\\n');os.fsync(fd);os.close(fd);s=os.stat(p);os.unlink(p);"
        "dd=os.open(d,os.O_RDONLY|os.O_DIRECTORY);os.fsync(dd);os.close(dd);"
        "print(json.dumps({'writer_uid':os.getuid(),'writer_gid':os.getgid(),"
        "'probe_uid':s.st_uid,'probe_gid':s.st_gid,'probe_mode_octal':format(s.st_mode&0o7777,'04o'),"
        "'create_fsync_unlink_pass':True,'directory_fsync_pass':True},sort_keys=True))"
    )
    completed = run(
        ["python3", "-c", probe_code, str(workspace)],
        as_user=account.pw_uid != os.geteuid(),
        timeout=30,
        log=log,
    )
    probe_path = workspace / ".s1c3-v7-ownership-probe"
    if probe_path.exists():
        raise GateFailure(f"oracle_ownership_probe_retained:{workspace.name}")
    try:
        result = json.loads(completed.stdout)
    except json.JSONDecodeError as error:
        raise GateFailure(f"oracle_ownership_probe_output_invalid:{workspace.name}") from error
    expected = {
        "writer_uid": account.pw_uid,
        "writer_gid": account.pw_gid,
        "probe_uid": account.pw_uid,
        "probe_gid": account.pw_gid,
        "probe_mode_octal": "0600",
        "create_fsync_unlink_pass": True,
        "directory_fsync_pass": True,
    }
    if result != expected:
        raise GateFailure(f"oracle_ownership_probe_invalid:{workspace.name}:{result}")
    return result
...
def make_oracle(
    ...
    shutil.copyfile(oracle_source, main_rs)
    shutil.copyfile(oracle_lock, cargo_lock)
    atomic_write(cargo_toml, oracle_manifest(source), 0o640)
    account = pwd.getpwnam("e")
    for directory in (crate, src):
        shutil.chown(directory, user=account.pw_uid, group=account.pw_gid)
        os.chmod(directory, 0o750)
    for file in (cargo_toml, cargo_lock, main_rs):
        shutil.chown(file, user=account.pw_uid, group=account.pw_gid)
        os.chmod(file, 0o640)
    probe_result = run_oracle_ownership_probe(
        crate,
        "e",
        evidence / f"oracle-ownership-{label}-probe.log",
    )
    row = {
        "schema": OWNERSHIP_ROW_SCHEMA,
        "label": label,
        "workspace": file_identity(crate),
        "src": file_identity(src),
        "cargo_toml": file_identity(cargo_toml),
        "main_rs": file_identity(main_rs),
        "main_rs_sha256": sha256_file(main_rs),
        "probe": probe_result,
        "probe_retained": False,
    }
    return cargo_toml, cargo_lock, row
...
def prebuild_oracle(
    ...
    manifest, copied_lock, ownership = make_oracle(
        oracle_source, oracle_lock, source, work, label, evidence
    )
    target = work / f"oracle-target-{label}"
    lock_before = sha256_file(copied_lock)
    if lock_before != ORACLE_LOCK_SHA256:
        raise GateFailure(f"oracle_lock_before_build_mismatch:{label}")
    command = [*ORACLE_CARGO_COMMAND_PREFIX, str(manifest)]
    environment = {**ORACLE_CARGO_ENVIRONMENT, "CARGO_TARGET_DIR": str(target)}
    completed = run(
        command,
        env=environment,
        as_user=True,
    ...
    if completed.returncode != 0:
        if offline_dependency_closure_missing(completed.stdout):
            raise GateFailure(f"OFFLINE_DEPENDENCY_CLOSURE_MISSING:{label}")
        tail = completed.stdout.decode("utf-8", "replace")[-2000:]
        raise GateFailure(f"oracle_build_failed:{label}:{completed.returncode}:{tail}")
    lock_after = sha256_file(copied_lock)
    if lock_after != ORACLE_LOCK_SHA256:
        raise GateFailure(f"oracle_lock_after_build_mismatch:{label}")
    executable = target / "release" / ORACLE_PACKAGE_NAME
    if not executable.is_file():
        raise GateFailure(f"parity_oracle_missing:{label}")
    ownership.update({
        "cargo_lock": oracle_lock_identity(copied_lock, lock_before, lock_after),
    ...
    ownership = add_root(ownership, "ownership_row_root_sha256")
    write_json(evidence / f"oracle-ownership-{label}.json", ownership)
    return executable, ownership
...
def build_ownership_receipt(
    ...
    if set(rows) != {"baseline", "candidate"}:
        raise GateFailure(f"oracle_ownership_labels_invalid:{sorted(rows)}")
    rows_root = sha256_bytes(canonical_bytes(rows))
    build_contract = {
    ...
    return add_root({
        ...
        "transaction_id": transaction_id,
        "build_user": {"name": "e", "uid": account.pw_uid, "gid": account.pw_gid},
        "rows": rows,
        "rows_root_sha256": rows_root,
        "oracle_build_contract": build_contract,
    ...
    for index, capture in enumerate((False, True)):
        ...
        with log_path.open("wb") as log:
            process = subprocess.Popen(
                ["taskset", "-c", str(selected_cpu), str(binary)],
                env=environment,
                stdout=log,
                stderr=subprocess.STDOUT,
                preexec_fn=demote,
...
def run_resources(
    ...
    for index in range(1, 4):
        output = run_test_binary(
            harnesses["response-actor"], source, HOT_TEST, f"hot-{index}", evidence, 600,
            monitor, selected_cpu
        )
    ...
    for index in range(1, 4):
        output = run_test_binary(
            ...
            f"single-sync-{index}", evidence, 900, monitor, selected_cpu
        )
    ...
    for index in range(1, 4):
        output = run_test_binary(
            ...
            f"three-sync-{index}", evidence, 900, monitor, selected_cpu
        )
        match = STAGE_SYNC_RE.search(output)
        if not match:
        ...
        three_runs.append({
            "precommit_p99_ns": precommit_p99,
            "precommit_hard_max_ns": precommit_hard_max,
            "settlement_p99_ns": settlement_p99,
            "settlement_hard_max_ns": settlement_hard_max,
            "episode_p99_ns": episode_p99,
            "episode_hard_max_ns": episode_hard_max,
            "samples": samples,
        })
    ...
    rss = measure_isolated_rss(
        candidate_binary, config, work, evidence, monitor, selected_cpu
    )
    all_pass = (
        ...
        and all(
            run["precommit_p99_ns"] <= 5_000_000
            and run["precommit_hard_max_ns"] <= 20_000_000
            and run["settlement_p99_ns"] <= 5_000_000
            and run["settlement_hard_max_ns"] <= 20_000_000
            and run["episode_hard_max_ns"] <= 20_000_000
            and run["samples"] == 256
...
def prepare(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if sha256_file(oracle_source) != ORACLE_SOURCE_SHA256:
            raise GateFailure("oracle_source_identity_drift")
        if sha256_file(oracle_lock) != ORACLE_LOCK_SHA256:
        ...
        projection_hash = production_projection_sha256(
            source / PRODUCTION_PROJECTION_PATH
        )
        ...
        baseline_oracle, baseline_ownership = prebuild_oracle(
            oracle_source, oracle_lock, baseline, work, "baseline", root / "evidence"
        )
        candidate_oracle, candidate_ownership = prebuild_oracle(
            oracle_source, oracle_lock, source, work, "candidate", root / "evidence"
        )
        oracles = {"baseline": baseline_oracle, "candidate": candidate_oracle}
        ownership = build_ownership_receipt(
            args.transaction_id,
            {"baseline": baseline_ownership, "candidate": candidate_ownership},
        )
        write_json(root / "oracle-ownership-receipt.json", ownership)
        if (root / "oracle-ownership-receipt.json").stat().st_mode & 0o7777 != 0o400:
            raise GateFailure("oracle_ownership_receipt_mode_invalid")
        ...
        quiescence = wait_for_quiescence(
            args.transaction_id,
            executable_identities,
            ownership["ownership_root_sha256"],
        )
        ...
        if selected_cpu not in MEASUREMENT_CPU_POOL:
            raise GateFailure("quiescence_selected_cpu_invalid")
        if any(
        ...
        if measurement_error is not None:
            write_json(
                ...
                {
                    "schema": "nando.s1c3-measurement-failure.v7",
                    "observed_at": utc_now(),
        ...
        write_json(root / "preflight-failure.json", {"schema": "nando.s1c3-preflight-failure.v7",
                                                      "observed_at": utc_now(), "error": str(error)})
        write_json(root / "transaction-state.json", {
            "schema": STATE_SCHEMA,
            "state": preflight_terminal_state(error),
            "transaction_id": args.transaction_id,
...
def execute(args: argparse.Namespace) -> int:
    root = Path(args.transaction_directory)
    preparation = read_json(root / "preparation.json")
    verification_path = Path(args.predeployment_verification)
    verification = read_json(verification_path)
    verification_root = verification.get("predeployment_verification_root_sha256")
    if not isinstance(verification_root, str) or len(verification_root) != 64:
        raise GateFailure("predeployment_verification_root_invalid")
    ...
    expected_verification = {
        ...
        "selected_cpu": preparation["measurement_cpu"],
        ...
        "predeployment_verification_root_sha256": verification_root,
    }
    if verification != expected_verification:
        raise GateFailure("predeployment_verification_binding_mismatch")
...
def finalize(args: argparse.Namespace) -> int:
    ...
    receipt = {
        ...
        "oracle_ownership_root_sha256": preparation["oracle_ownership_root_sha256"],
        "quiescence_root_sha256": preparation["quiescence_root_sha256"],
        "measurement_cpu": preparation["measurement_cpu"],
        "measurement_contamination_root_sha256": preparation[
            "measurement_contamination_root_sha256"
        ],
        ...
        "resource_root_sha256": preparation["resource_root_sha256"],
        "parity_root_sha256": preparation["parity_root_sha256"],
        "predeployment_verification_root_sha256": read_json(
            root / "predeployment-verification.json"
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "nando.s1c3-executor-error.v7", "error": str(error)}, sort_keys=True), file=sys.stderr)
        return 2
