...
use super::{
    ...
    K2_UNCERTAINTY_R8B_STDOUT_RECEIPT_PATH_V2, K2UncertaintyAuthorizationSlotLedgerV1,
    ...
    K2UncertaintyGeneratorResponseV1, K2UncertaintyR8BExecutableIdentityV2,
    K2UncertaintyR8BLedgerWriterV2, K2UncertaintyR8BLedgerWriterV3, K2UncertaintyR8BProcessEventV3,
    K2UncertaintyR8BProducedReceiptV2, K2UncertaintyR8BProducerRequestV3,
    K2UncertaintyR8BValidatedFactV3, load_confirm_generator_split_receipt_v1,
...
fn generator_writer_role_v3() -> K2CompositionResultV1<&'static str> {
    ...
    let name = owner
        ...
        .and_then(|value| value.to_str())
        .unwrap_or_default();
    if name == "nando-k2-self-formed-confirm-owner" {
        Ok("M01_DEVELOPMENT_OWNER")
    } else if name.starts_with("k2_self_formed_uncertainty_confirm_r8b_restart_v1-") {
        Ok("S02_RESTART")
    } else {
        Err(K2CompositionErrorV1::Invalid(
            "self_formed_r8b_v3_generator_owner_invalid",
        ))
    }
}
...
> {
    ...
    let bytes = fs::read(path)
        .map_err(|_| K2CompositionErrorV1::Io("read_self_formed_r8b_v3_producer_request"))?;
    ...
    if uncertainty_bytes_v1(&request)? != bytes {
        return Err(K2CompositionErrorV1::Invalid(
            "self_formed_r8b_v3_request_bytes_invalid",
        ));
    }
    ...
    let plan = request
        ...
        .find(|row| {
            row.request_owner_role == owner
                && row.target_role == "M02_GENERATOR"
                && row.stage == "C02"
                && !observed.contains(&row.invocation_id_sha256)
        })
        .cloned()
        .ok_or(K2CompositionErrorV1::Invalid(
            "self_formed_r8b_v3_generator_plan_missing",
        ))?;
