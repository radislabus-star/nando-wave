...
pub(super) fn read_bounded_jsonl_line_v1<R: BufRead>(
    ...
    loop {
        let available = reader
            .fill_buf()
            .map_err(|_| K2CompositionErrorV1::Io("read_self_formed_bounded_jsonl"))?;
    ...
    F: Fn(File) -> K2CompositionResultV1<T>,
{
    ...
    let metadata = file
        .metadata()
        .map_err(|_| K2CompositionErrorV1::Io("stat_self_formed_renamed_file"))?;
...
pub(super) fn read_closed_file_v2(
    ...
    if expected_content_sha256
        .is_some_and(|expected| composition_sha256_bytes_v1(&bytes) != expected)
    {
...
pub(super) fn open_closed_file_v2(
    ...
    let metadata = file
        .metadata()
        .map_err(|_| K2CompositionErrorV1::Io("stat_self_formed_closed_file"))?;
    if !metadata.is_file()
        || metadata.nlink() != 1
...
pub(super) fn open_nofollow_file_v1(
    path: &Path,
    write: bool,
    append: bool,
) -> K2CompositionResultV1<File> {
...
pub(super) fn create_exclusive_file_v1(
    path: &Path,
    bytes: &[u8],
    mode: u32,
) -> K2CompositionResultV1<()> {
...
pub(super) fn validate_regular_file_v1(
    file: &File,
    mode: u32,
    max_bytes: u64,
) -> K2CompositionResultV1<()> {
    let metadata = file
        .metadata()
        .map_err(|_| K2CompositionErrorV1::Io("stat_self_formed_regular_file"))?;
    if !metadata.is_file()
        || metadata.nlink() != 1
...
pub(super) fn rename_noreplace_same_device_v1(
    source: &Path,
    destination: &Path,
) -> K2CompositionResultV1<()> {
    let destination_parent = destination
        .parent()
        .ok_or_else(|| invalid_v1("self_formed_rename_destination_invalid"))?;
    if fs::metadata(source)
        .map_err(|_| invalid_v1("self_formed_rename_source_stat"))?
        .dev()
        != fs::metadata(destination_parent)
            .map_err(|_| invalid_v1("self_formed_rename_destination_stat"))?
            .dev()
    {
