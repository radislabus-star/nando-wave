...
fn discover_clean_applicability_subcenters(
    ...
    for frame in &positives {
        let hidden = crate::relation_frame_hidden_wave_atom_ids(frame)
            .into_iter()
            .collect::<BTreeSet<_>>();
        ...
        let (anti_center_atom_sets, learned_wave_route) = match derive_safe_anti_centers(
            ...
            Err(blocker) => {
                let phase_negatives = state
                    ...
                    .cloned()
                    .collect::<Vec<_>>();
                let learned = crate::learned_wave_route_from_support_medoid(
                    &state.positives,
                    &phase_negatives,
                    32,
                );
                let phase_separates = learned.as_ref().is_some_and(|route| {
                    state
                        .positives
                        .iter()
                        .all(|frame| crate::learned_wave_route_accepts_frame(route, frame))
                        && phase_negatives
                            .iter()
                            .all(|frame| !crate::learned_wave_route_accepts_frame(route, frame))
                });
...
#[must_use]
#[doc(hidden)]
pub fn winner_routes_frame(winner: &CegisWinner, frame: &crate::RelationFrame) -> bool {
    frame_has_atoms(frame, &winner.required_atom_ids)
        ...
        && winner
            .learned_wave_route
            .as_ref()
            .is_none_or(|route| crate::learned_wave_route_accepts_frame(route, frame))
