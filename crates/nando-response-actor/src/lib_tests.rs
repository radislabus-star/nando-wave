...
fn p3_freeze_reserves_recent_sessions_without_starving_support() {
    ...
    let policy = ResponseSupportFreezePolicy {
        ...
        generation_by_lineage: std::collections::BTreeMap::from([(manifest.lineage_id.clone(), 2)]),
...
fn p2_verifier_mutations_and_ambiguous_roles_are_killed() {
    ...
    let operator =
        synthesize_response_operator(std::slice::from_ref(&frame)).expect("synthesize operator");
