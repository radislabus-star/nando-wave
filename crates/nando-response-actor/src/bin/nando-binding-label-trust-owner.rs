...
use nando_response_actor::seal_binding_external_label_trust_v1;
...
fn main() -> Result<(), String> {
    ...
    let receipt = seal_binding_external_label_trust_v1(
        &preregistration,
        &b1a_report,
        &support_freeze,
        &support_watermark,
    ...
    .map_err(|error| format!("external_trust_owner:{error:?}"))?;
    write_new_sync(
        &output,
        &receipt
            .canonical_bytes()
            .map_err(|error| format!("external_trust_encode:{error:?}"))?,
    )?;
    Ok(())
}
...
fn usage() -> String {
    "usage: nando-binding-label-trust-owner <preregistration.json> <b1a-report.json> <support-freeze.json> <support-watermark.json> <future-freeze.json> <future-external-receipt.json> <physical-receipts.json> <label-manifest.json> <trust-receipt.json>".to_owned()
}
