...
const MAX_PARITY_PAYLOAD_BYTES: u64 = 64 * 1024 * 1024;
...
impl CompactRuntimeParityCase {
    fn from_runtime(case: crate::RuntimeParityCase) -> Option<Self> {
        let provider_payload_json = serde_json::to_vec(&case.provider_payload).ok()?;
        let provider_payload_zstd = Self::compress_payload(&provider_payload_json)?;
        Some(Self {
    ...
    fn compact_in_place(&mut self) -> bool {
        if self.provider_payload_zstd.is_empty() {
            let json = if let Some(json) = self.provider_payload_json.as_deref() {
                json.as_bytes().to_vec()
            } else if let Some(payload) = self.provider_payload.as_ref() {
            ...
            let Some(compressed) = Self::compress_payload(&json) else {
                return false;
            };
            self.provider_payload_zstd = compressed;
        }
    ...
    fn materialize(&self) -> Option<crate::RuntimeParityCase> {
        let provider_payload = if !self.provider_payload_zstd.is_empty() {
            let decoder =
                zstd::stream::read::Decoder::new(self.provider_payload_zstd.as_slice()).ok()?;
            let mut json = Vec::new();
            decoder
                .take(MAX_PARITY_PAYLOAD_BYTES.saturating_add(1))
                .read_to_end(&mut json)
                .ok()?;
            if u64::try_from(json.len()).ok()? > MAX_PARITY_PAYLOAD_BYTES {
                return None;
            }
            serde_json::from_slice(&json).ok()?
        } else if let Some(json) = self.provider_payload_json.as_deref() {
    ...
    fn compress_payload(json: &[u8]) -> Option<Vec<u8>> {
        if u64::try_from(json.len()).ok()? > MAX_PARITY_PAYLOAD_BYTES {
            return None;
        }
        zstd::stream::encode_all(json, 1).ok()
    }
}
...
mod tests {
    ...
    fn compact_parity_reads_legacy_payload_and_materializes_exactly() {
        ...
        assert!(!compact.provider_payload_zstd.is_empty());
        assert!(compact.provider_payload_json.is_none());
        ...
        assert!(stored.get("provider_payload_json").is_none());
        assert!(stored.get("provider_payload_zstd").is_some());
    }
