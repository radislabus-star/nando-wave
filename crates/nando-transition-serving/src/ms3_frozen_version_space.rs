...
impl Ms3FrozenVersionSpaceRuntime {
    ...
    pub(super) fn seal_ineligible_probe_censor(
        &mut self,
        report: &nando_operator_learning::multi_source::Ms3LinkedFrameAcquisitionReportV1,
        closure_capture_sequence: u64,
    ) -> Result<
    ...
    > {
        if self.envelope.is_some() || self.independent_future.is_some() {
            return Err("ms3_unattributed_probe_censor_after_freeze".to_owned());
        }
        let mut generation_registry = self.generation_registry.clone();
        let receipt = generation_registry
            .seal_ineligible_probe_censor(
                self.generation_sequence,
                report,
                closure_capture_sequence,
            )
            .map_err(|error| {
                format!("ms3_generation_registry_ineligible_probe_censor:{error:?}")
            })?;
        let registry_bytes = generation_registry
            .canonical_bytes()
            .map_err(|error| format!("ms3_generation_registry_encode:{error:?}"))?;
    ...
    pub(super) fn seal_linked_evidence_reuse(
        &mut self,
        report: &nando_operator_learning::multi_source::Ms3LinkedFrameAcquisitionReportV1,
        closure_capture_sequence: u64,
    ) -> Result<
    ...
    > {
        if self.envelope.is_some() || self.independent_future.is_some() {
            return Err("ms3_linked_evidence_reuse_after_freeze".to_owned());
        }
        let mut generation_registry = self.generation_registry.clone();
        let receipt = generation_registry
            .seal_linked_evidence_reuse(self.generation_sequence, report, closure_capture_sequence)
            .map_err(|error| format!("ms3_generation_registry_linked_evidence_reuse:{error:?}"))?;
        let registry_bytes = generation_registry
            .canonical_bytes()
            .map_err(|error| format!("ms3_generation_registry_encode:{error:?}"))?;
        write_atomic(&self.generation_registry_path, &registry_bytes)?;
