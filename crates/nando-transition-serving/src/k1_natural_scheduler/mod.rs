...
pub(crate) fn candidate_program_binding_root(
    freeze: &K1NaturalCandidateFreezeV1,
    ...
    if matches!(
        ...
        let motif = source_neutral_topology_motifs_v1(topology)
            .map_err(str::to_owned)?
            .into_iter()
            .find(|motif| motif.motif_root_sha256 == freeze.candidate_structural_root_sha256)
            .ok_or_else(|| "k1_candidate_motif_topology_mismatch".to_owned())?;
        return bind_pre_action_t1_program_to_motif_v1(program, topology, &motif)
            .map(|binding| binding.binding_root_sha256)
            .map_err(str::to_owned);
    }

    if pre_action_applicability_shape_root_v1(topology).map_err(str::to_owned)?
        != freeze.candidate_structural_root_sha256
        || candidate_topology_root(freeze, topology)? != freeze.source_neutral_topology_root_sha256
    {
        return Err("k1_candidate_topology_mismatch".to_owned());
    }
    pre_action_t1_binding_root(program, topology).map_err(str::to_owned)
}
