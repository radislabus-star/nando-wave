...
use crate::{
    ...
    authorized, exact_current_epoch_token_totals, exact_miner_opportunity, metric_str, metric_u64,
    not_found, read_json, read_live_json, read_live_miner_report, unix_now,
};
use axum::Json;
...
pub(crate) async fn control_signal_path(
    ...
    let token_totals = exact_current_epoch_token_totals(economics);
    let miner_opportunity = exact_miner_opportunity(&live, &persisted_miner);
    ...
    let snapshot = evaluation::build(SignalPathInputs {
        ...
        miner_input_tokens: miner_opportunity.map(|value| metric_u64(value, "ordinary_tokens")),
        miner_cpu_verified_input_tokens: miner_opportunity
            .map(|value| metric_u64(value, "verified_tokens")),
        cpu_input_tokens: token_totals.map(|(_, cpu)| cpu),
        accounting_epoch_total_input_tokens: token_totals.map(|(total, _)| total),
        accounting_epoch_cpu_input_tokens: token_totals.map(|(_, cpu)| cpu),
