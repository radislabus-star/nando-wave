...
async fn control_dashboard_snapshot(
    ...
    let operator_certificates =
        build_operator_certificate_matrix(response_packages, &ms4_closed_loop);
    let (queue_rows, ready_now, readiness_blockers) = scheduler_readiness(&k1_scheduler);
    let projection = k1_scheduler
        .get("projection")
        .filter(|value| value.is_object())
        .unwrap_or(&Value::Null);
    ...
    (
        ...
        Json(json!({
            ...
            "discovery": {
                ...
                "catalog_cohorts": metric_u64(&natural_vocabulary_census, "cohorts_total"),
                ...
                "leading_candidate_root_sha256": natural_vocabulary_census
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or(""),
                "completed_generations": metric_u64(projection, "completed_generations"),
                "next_generation_sequence": metric_u64(projection, "next_generation_sequence"),
                "active_candidate": projection.get("active_candidate_freeze").is_some_and(|value| !value.is_null()),
                "latest_verdict": projection.pointer("/latest_terminal_verdict/verdict").and_then(Value::as_str).unwrap_or(""),
                "latest_verdict_blocker": projection.pointer("/latest_terminal_verdict/blocker").and_then(Value::as_str).unwrap_or(""),
...
fn scheduler_readiness(scheduler: &Value) -> (u64, u64, BTreeMap<String, u64>) {
    let queue = scheduler
        .pointer("/queue/rows")
        .and_then(Value::as_array)
        .map(Vec::as_slice)
    ...
    let ready_now = queue
        .iter()
        .filter(|row| row.pointer("/score/readiness_rank").and_then(Value::as_u64) == Some(1))
        .count() as u64;
    let mut blockers = BTreeMap::new();
    for candidate in scheduler
        .pointer("/catalog/candidates")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
    {
        let blocker = candidate
            .pointer("/readiness/blocker")
            .and_then(Value::as_str)
            .unwrap_or_default();
        if !blocker.is_empty() {
    ...
    (queue.len() as u64, ready_now, blockers)
}
...
mod tests {
    ...
    #[test]
    fn compact_dashboard_separates_historical_readiness_from_schedulable_rows() {
        let scheduler = json!({
            "catalog": {"candidates": [
                {"readiness": {"pass": true, "blocker": ""}},
                {"readiness": {"pass": false, "blocker": "settled_evidence_below_freeze_minimum"}},
                {"readiness": {"pass": false, "blocker": "settled_evidence_below_freeze_minimum"}},
                {"readiness": {"pass": false, "blocker": "independent_lineages_below_freeze_minimum"}}
            ]},
            "queue": {"rows": [
                {"score": {"readiness_rank": 0}},
                {"score": {"readiness_rank": 1}}
            ]}
        });
        let (queue_rows, ready_now, blockers) = scheduler_readiness(&scheduler);
        assert_eq!(queue_rows, 2);
