...
const DASHBOARD_BUILD: &str = "2026.08.09-b049";
...
<main class="nando-live" data-dashboard-build="__DASHBOARD_BUILD__" aria-label="Nando live traffic control">
  ...
  <section class="live-band"><div class="live-inner">
    ...
    <div class="ms3-grid">
      ...
      <div class="ms3-cell"><div class="ms3-label">RAW PHASE EXECUTABLE / VERSION SPACE</div><div id="k1-scheduler-version-space" class="ms3-value watch">NOT IDENTIFIED</div><div id="k1-scheduler-identification" class="scope-note">IDENTIFICATION —</div><div id="k1-scheduler-raw-phase" class="scope-note">EXECUTABLE BUILDER READY · WAITING FOR FROZEN GENERATION</div></div>
...
(() => {
  ...
  const renderTokens = (snapshot) => {
    ...
    const vocabularyCensus = snapshot.natural_vocabulary_census || {};
    ...
    const k1RawPhaseExecutableRoot = k1Identification.raw_phase_executable_blueprint_root_sha256 || "";
    const k1RawPhaseSelected = k1Identification.raw_phase_selected_executable || {};
    ...
    const k1RawPhaseStatus = k1RawPhaseSelected.receipt_root_sha256
      ? `SELECTED EXECUTABLE · SUPPORT ${number.format((k1RawPhaseSelected.support_evidence || []).length)} · BLUEPRINTS ${number.format((k1RawPhaseSelected.selected_disposition?.blueprint_fingerprints_sha256 || []).length)} · RECEIPT ${k1RawPhaseSelected.receipt_root_sha256}`
      : k1RawPhaseExecutableRoot
        ? `EXECUTABLE ENVELOPE · ${number.format(k1Identification.raw_phase_executable_programs || 0)} EXECUTABLE · ${number.format(k1Identification.raw_phase_excluded_programs || 0)} EXCLUDED · ROOT ${k1RawPhaseExecutableRoot}`
        : k1RawPhaseRoot
          ? `HYPOTHESIS FROZEN · CANDIDATES ${number.format(k1Identification.candidate_programs || 0)} · SUPPORT ≤ ${number.format(k1Identification.raw_phase_support_watermark || 0)} · ROOT ${k1RawPhaseRoot}`
          : "EXECUTABLE BUILDER READY · WAITING FOR FROZEN GENERATION";
    text("k1-scheduler-raw-phase", k1RawPhaseStatus);
    ...
    const bindingGapForm = vocabularyForms
      .filter(form => form.missing_from_current_vocabulary !== true)
      .sort((left, right) => ((right.joined_rows || 0) - (right.source_neutral_candidate_rows || 0)) - ((left.joined_rows || 0) - (left.source_neutral_candidate_rows || 0)))[0] || {};
    const vocabularyVerdict = vocabularyCensusAvailable
      ? (vocabularyCensus.verdict || "unknown").replaceAll("_", " ").toUpperCase()
      : "CENSUS UNAVAILABLE";
    const vocabularyBlocker = vocabularyCensusAvailable
      ? (vocabularyCensus.blocker || "NONE").replaceAll("_", " ").toUpperCase()
      : "REPORT MISSING";
    const missingFormName = (missingVocabularyForm.operation_form || "none").replaceAll("_", " ").toUpperCase();
    const bindingGap = Math.max(0, (bindingGapForm.joined_rows || 0) - (bindingGapForm.source_neutral_candidate_rows || 0));
    text("vocabulary-census-status", vocabularyVerdict);
    ...
    text("next-route", k1VocabularyOpen
      ...
      ? `Capture V2 ${(missingVocabularyForm.operation_form || "missing form").replaceAll("_", " ")} → readiness → Law #2/#3 → K1 OPEN → natural L2`
...
mod tests {
    ...
    fn render_leads_with_total_server_miner_recognition_and_cpu() {
        ...
        assert!(html.contains("RAW PHASE EXECUTABLE / VERSION SPACE"));
        assert!(html.contains("SELECTED EXECUTABLE · SUPPORT"));
        assert!(html.contains("EXECUTABLE ENVELOPE"));
        assert!(html.contains("HYPOTHESIS FROZEN · CANDIDATES"));
        assert!(html.contains("EXECUTABLE BUILDER READY · WAITING FOR FROZEN GENERATION"));
        assert!(html.contains("raw_phase_hypothesis_root_sha256"));
        assert!(html.contains("raw_phase_executable_blueprint_root_sha256"));
        assert!(html.contains("raw_phase_selected_executable"));
