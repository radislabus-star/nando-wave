...
const DEFAULT_READ_INSPECT_LOCAL_ACCEPT_CALIBRATION_REPORT: &str =
...
const DEFAULT_GIT_CONTROL_PACKAGE_PATH: &str =
    "target/nando-wave/real-traffic-shadow/git-control-seed0.nwrb";
const DEFAULT_GIT_CONTROL_PROFILE_REGISTRY_CONFIG: &str =
...
const REAL_TRAFFIC_READ_INSPECT_PATH_ROLE_SLOT: u8 = 0;
const REAL_TRAFFIC_READ_INSPECT_REQUEST_ROLE_SLOT: u8 = 1;
const REAL_TRAFFIC_READ_INSPECT_EVIDENCE_ROLE_SLOT: u8 = 2;
const REAL_TRAFFIC_READ_INSPECT_CHECK_ROLE_SLOT: u8 = 3;
const REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_SHIFT: u32 = 5;
const REAL_TRAFFIC_READ_INSPECT_TOP_ROLE_L1_LANES: usize = 32;
const REAL_TRAFFIC_READ_INSPECT_STATE_DELTA_LANES_PER_SIDE: usize = 24;
const REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY: &str = "read_inspect";
...
const REAL_TRAFFIC_GIT_CONTROL_INTENT_ROLE_SLOT: u8 = 0;
const REAL_TRAFFIC_GIT_CONTROL_COMMAND_ROLE_SLOT: u8 = 1;
const REAL_TRAFFIC_GIT_CONTROL_TARGET_ROLE_SLOT: u8 = 2;
const REAL_TRAFFIC_GIT_CONTROL_SAFETY_ROLE_SLOT: u8 = 3;
const REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_SHIFT: u32 = 5;
const REAL_TRAFFIC_GIT_CONTROL_TOP_ROLE_L1_LANES: usize = 32;
const REAL_TRAFFIC_GIT_CONTROL_STATE_DELTA_LANES_PER_SIDE: usize = 24;
const REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY: &str = "git_control";
    ...
    I: Iterator<Item = String>,
{
    ...
    let report_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_READ_INSPECT_PAYLOAD_DRY_RUN_REPORT));
    let max_events = args
        .next()
        .map(|value| {
            value
    ...
    let profile_registered = registry_config
        .profiles
        .iter()
        .any(|profile| profile.profile_id == REAL_TRAFFIC_READ_INSPECT_PROFILE_ID);
    let route_catalog = CodexHistoryRouteCatalog::from_registry(&registry_config)?;
    let history_rows = read_codex_history_jsonl(&history_path)?;
    let skip = history_rows.len().saturating_sub(max_events);
    let mut trace_rows = Vec::with_capacity(history_rows.len().saturating_sub(skip));
    let mut report_rows = Vec::new();
    let mut read_inspect_candidate_events = 0usize;
    let mut payload_ready_events = 0usize;
    let mut payload_built_events = 0usize;
    let mut scoreable_payload_events = 0usize;
    let mut builder_rejected_events = 0usize;
    ...
    for (index, row) in history_rows.iter().enumerate().skip(skip) {
        let fingerprint = stable_real_traffic_fingerprint64(row.text.as_bytes());
        let event_id = format!(
            "codex_history_read_inspect_payload_dry_run::{}::{}::{}",
            row.session_id, row.ts, index
        );
        let request_fingerprint = format!("fnv1a64:{fingerprint:016x}");
        let exact_cache_key = Some(format!("codex_history_request:{fingerprint:016x}"));
        let mut nando_shadow_request = None;
        let mut notes = "not read_inspect route-gap candidate".to_owned();

        if route_catalog.classify_request_text(&row.text).is_none()
            && route_gap_family_key(&row.text) == REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY
        {
            read_inspect_candidate_events += 1;
            let readiness =
                analyze_route_gap_payload_readiness(REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY, &row.text);
            if readiness.payload_ready {
                payload_ready_events += 1;
                let built = build_read_inspect_dry_run_request(&event_id, &fingerprint, &row.text);
                match built {
                    Some(request) => {
                        let active_fringe_centers = request.active_fringe.len();
                        let slots = request.slots.len();
                        ...
                        *builder_status_counts
                            .entry(builder_status.clone())
                            .or_insert(0) += 1;
                        report_rows.push(RoleBindingReadInspectPayloadDryRunRow {
                            event_id: event_id.clone(),
                            request_fingerprint: request_fingerprint.clone(),
                            route_key: REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY.to_owned(),
                            profile_id: REAL_TRAFFIC_READ_INSPECT_PROFILE_ID.to_owned(),
                            readiness_payload_ready: true,
                            payload_built: true,
                            scoreable,
                            profile_registered,
                        ...
                        notes = format!(
                            "request-side read-inspect payload built; status={builder_status}; verified accepts disabled"
                        );
                        nando_shadow_request = Some(request);
                    }
                    None => {
                ...
                *builder_status_counts
                    ...
                    .or_insert(0) += 1;
                notes = format!(
                    "read_inspect route-gap candidate rejected by readiness gate: {}",
                    readiness.missing_reasons.join(",")
                );
            }
        }
        ...
        trace_rows.push(RoleBindingRealTrafficTraceRow {
            schema_version: "nando_role_binding_real_traffic_trace_v1".to_owned(),
            trace_id: event_id,
            traffic_source: Some("codex_history_local_read_inspect_payload_dry_run".to_owned()),
            time_ms: Some(row.ts.saturating_mul(1000)),
            request_fingerprint: Some(request_fingerprint),
            response_fingerprint: None,
            tool_call_fingerprints: Vec::new(),
            verification_source: Some(
                "request-side read-inspect payload dry-run from local Codex prompt only; raw text, response text, target labels, and proof labels not written"
                    .to_owned(),
            ),
            llm_call: true,
            exact_cache_key,
    ...
    write_real_traffic_trace_jsonl(&trace_path, &trace_rows)?;
    let shadow_score_ready = profile_registered && scoreable_payload_events > 0;
    let report = RoleBindingReadInspectPayloadDryRunReport {
        schema_version: "nando_role_binding_read_inspect_payload_dry_run_v1".to_owned(),
        verdict: if shadow_score_ready {
            "READ_INSPECT_PAYLOAD_DRY_RUN_V1_REVIEW_SCOREABLE_PROFILE_READY"
        } else if scoreable_payload_events > 0 {
            "READ_INSPECT_PAYLOAD_DRY_RUN_V1_REVIEW_SCOREABLE_PAYLOADS_PROFILE_MISSING"
        } else {
            "READ_INSPECT_PAYLOAD_DRY_RUN_V1_REVIEW_NO_SCOREABLE_PAYLOADS"
        }
        .to_owned(),
        history_path: history_path.display().to_string(),
        registry_config_path: registry_config_path.display().to_string(),
        ...
        market_claim_allowed: false,
        rows: report_rows,
        claim_boundary: "Request-side dry-run payload builder only. It emits active_fringe/slots for read_inspect route-gap rows from prompt text only, keeps verified_safe_accept=None and expect_local_operator=false, writes no raw prompt text, and cannot prove savings. A read-only path/excerpt verifier is required before any local accept.".to_owned(),
        next_engineering_debt: "Compile a read_inspect .nwrb scoring profile, rerun shadow, then attach read_only_path_and_excerpt_verifier_v1 before threshold calibration or market claim.".to_owned(),
    };
    write_json_file(&report_path, &report)?;
    println!(
    ...
    println!(
        "  read_inspect_candidate_events: {}",
        report.read_inspect_candidate_events
    );
    println!("  payload_ready_events: {}", report.payload_ready_events);
    println!("  payload_built_events: {}", report.payload_built_events);
    println!(
    ...
    println!("  local_accepts_enabled: false");
    Err(
        "read-inspect payload dry-run is review-only; build profile+verifier before claims"
            .to_owned(),
    )
}
    ...
    I: Iterator<Item = String>,
{
    ...
    let package_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_READ_INSPECT_PACKAGE_PATH));
    let registry_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_READ_INSPECT_PROFILE_REGISTRY_CONFIG));
    let report_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_READ_INSPECT_PROFILE_REPORT));

    let mut registry = read_json_file::<RoleBindingProfileRegistryConfig>(&base_registry_path)?;
    validate_registry_config(&registry)?;
    let trace_rows = read_real_traffic_trace_jsonl(&dry_run_trace_path)?;
    let build = build_read_inspect_role_binding_package_from_trace(&trace_rows)?;
    if let Some(parent) = package_path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            format!(
                "failed to create read-inspect package directory {}: {error}",
                parent.display()
            )
        })?;
    }
    fs::write(&package_path, &build.package_bytes).map_err(|error| {
        format!(
            "failed to write read-inspect package {}: {error}",
            package_path.display()
        )
    })?;
    let package_info =
        WavePredictorRoleBindingOffloadRuntime::inspect_package_bytes(&build.package_bytes)
            .map_err(|error| format!("failed to inspect read-inspect package: {error:?}"))?;
    let policy =
        WavePredictorRoleBindingOffloadPolicy::new(REAL_TRAFFIC_READ_INSPECT_DISABLED_THRESHOLD)
            .map_err(|error| format!("invalid read-inspect disabled policy: {error:?}"))?;
    let sdk = WavePredictorRoleBindingOffloadRuntime::from_package_bytes_serving_packed_only(
        &build.package_bytes,
        policy,
    )
    .map_err(|error| format!("failed to load read-inspect package: {error:?}"))?;

    let requests = read_inspect_scoreable_requests(&trace_rows);
    let mut energy_margins = Vec::with_capacity(requests.len());
    let mut min_slot_margins = Vec::with_capacity(requests.len());
    let mut positive_margin_rows = 0usize;
    let mut strict_ordered_pass_rows = 0usize;
    ...
    let profile = RoleBindingProfileConfig {
        ...
        edge_count: package_info.edge_count,
        slot_count: 3,
        threshold: REAL_TRAFFIC_READ_INSPECT_DISABLED_THRESHOLD,
        acceptance_policy: default_profile_acceptance_policy(),
        accepted_route_keys: vec![
            REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY.to_owned(),
            REAL_TRAFFIC_READ_INSPECT_PROFILE_ID.to_owned(),
    ...
    registry
        ...
        .retain(|existing| existing.profile_id != profile.profile_id);
    registry.profiles.push(profile);
    registry.claim_boundary = "serving registry overlay for read_inspect .nwrb profile; generated from request-side dry-run payloads with threshold=i32::MAX so scoring telemetry is available but local accepts remain disabled until read-only path/excerpt verification exists".to_owned();
    validate_registry_config(&registry)?;
    write_json_file(&registry_path, &registry)?;
    ...
    sorted_energy.sort_unstable();
    sorted_min_slot.sort_unstable();
    let report = RoleBindingReadInspectProfileReport {
        schema_version: "nando_role_binding_read_inspect_profile_v1".to_owned(),
        verdict: if unexpected_local_accepts_under_disabled_threshold == 0
            && build.package_training_requests > 0
            && package_info.edge_count > 0
        {
            "READ_INSPECT_PROFILE_V1_REVIEW_PROFILE_READY_ACCEPTS_DISABLED"
        } else {
            "READ_INSPECT_PROFILE_V1_REVIEW_REPAIR_REQUIRED"
        }
        .to_owned(),
        base_registry_path: base_registry_path.display().to_string(),
        dry_run_trace_path: dry_run_trace_path.display().to_string(),
        ...
        local_accepts_enabled_on_real_traffic: false,
        market_claim_allowed: false,
        claim_boundary: "Profile generator only. It compiles request-side read_inspect payload geometry into a .nwrb package and registry overlay with threshold=i32::MAX, so shadow can measure real score/margins but cannot local-accept. Verified CPU savings require read-only path/excerpt evidence, safe-policy calibration, shadow/audit pass, provider cost, and false_accepts=0.".to_owned(),
        next_engineering_debt: "Run read_inspect dry-run shadow with this overlay registry, then build read_only_path_and_excerpt_verifier_v1 before lowering thresholds or promoting any local accept path.".to_owned(),
    };
    write_json_file(&report_path, &report)?;
    println!(
...
pub(crate) fn run_role_binding_real_traffic_read_inspect_local_accept_calibration_v1<I>(
    mut args: I,
) -> Result<(), String>
where
    I: Iterator<Item = String>,
{
    ...
    let report_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_READ_INSPECT_LOCAL_ACCEPT_CALIBRATION_REPORT));
    ...
    for row in &trace_rows {
        ...
        let current_response = score_role_binding_profile_request(&registry, request);
        let path_slot_margin = score.slot_margins.first().copied().unwrap_or(0);
        let request_slot_margin = score.slot_margins.get(1).copied().unwrap_or(0);
        scored_rows.push(RoleBindingEditLocalAcceptCalibrationRow {
            trace_id: row.trace_id.clone(),
            request_fingerprint: row.request_fingerprint.clone(),
            response_fingerprint: row.response_fingerprint.clone(),
            ...
            energy_margin: score.energy_margin,
            min_slot_margin: score.min_slot_margin,
            marker_slot_margin: path_slot_margin,
            end_slot_margin: request_slot_margin,
            slot_count: score.slot_margins.len(),
        });
    }
    ...
    let path_slot_policy =
        evaluate_edit_calibration_policy("path_slot_positive_only", &scored_rows, |row| {
            row.marker_slot_margin > 0 && row.energy_margin >= 1
        });
    let request_slot_policy =
        evaluate_edit_calibration_policy("request_slot_positive_only", &scored_rows, |row| {
            row.end_slot_margin > 0 && row.energy_margin >= 1
        });
    let best_energy_threshold_policy =
        best_single_threshold_policy("best_energy_margin_threshold", &scored_rows, |row| {
    ...
    let best_path_slot_threshold_policy =
        best_single_threshold_policy("best_path_slot_margin_threshold", &scored_rows, |row| {
            row.marker_slot_margin
        });
    let best_request_slot_threshold_policy =
        best_single_threshold_policy("best_request_slot_margin_threshold", &scored_rows, |row| {
            row.end_slot_margin
        });
    let margin_collision_diagnostics = planning_margin_collision_diagnostics(&scored_rows);
    let request_side_margin_only_accepts_all_true_without_false = margin_collision_diagnostics
    ...
    let policies = vec![
        ...
        best_energy_threshold_policy,
        best_min_slot_threshold_policy,
        best_path_slot_threshold_policy,
        best_request_slot_threshold_policy,
    ];
    let safe_policy_found = policies
        .iter()
        .any(|policy| policy.false_accepts == 0 && policy.true_accepts > 0);
    let best_safe_true_accepts = policies
        ...
        .unwrap_or(0);
    let report = RoleBindingEditLocalAcceptCalibrationReport {
        schema_version: "nando_role_binding_read_inspect_local_accept_calibration_v1"
            .to_owned(),
        verdict: if safe_policy_found {
            "READ_INSPECT_LOCAL_ACCEPT_CALIBRATION_V1_REVIEW_SAFE_POLICY_CANDIDATE_FOUND"
        } else {
        ...
        local_accepts_enabled: false,
        market_claim_allowed: false,
        claim_boundary: "Read-inspect calibration only. It evaluates request-side score/readout policies against deterministic read-only path/excerpt verifier labels, writes fingerprints and margins only, enables no local accepts, and cannot be used as a market savings claim.".to_owned(),
        next_engineering_debt: if safe_policy_found {
            "Promote only through a separate read-inspect safe-policy artifact, then rerun shadow/audit with provider cost, false_accepts=0, and unverified_shadow_accepts=0 before counting savings.".to_owned()
        } else {
            "Do not lower the read-inspect threshold. Current score geometry does not separate verifier-true from verifier-false rows; improve request-side admission or payload features before enabling local accepts.".to_owned()
        },
    };
    write_json_file(&report_path, &report)?;
    println!(
    ...
    Err("read-inspect local accept calibration is review-only".to_owned())
}

pub(crate) fn run_role_binding_real_traffic_git_control_payload_dry_run_v1<I>(
    mut args: I,
) -> Result<(), String>
where
    I: Iterator<Item = String>,
{
    ...
    let report_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_GIT_CONTROL_PAYLOAD_DRY_RUN_REPORT));
    let max_events = args
        .next()
        .map(|value| {
            value
    ...
    let profile_registered = registry_config
        .profiles
        .iter()
        .any(|profile| profile.profile_id == REAL_TRAFFIC_GIT_CONTROL_PROFILE_ID);
    let route_catalog = CodexHistoryRouteCatalog::from_registry(&registry_config)?;
    let history_rows = read_codex_history_jsonl(&history_path)?;
    let skip = history_rows.len().saturating_sub(max_events);
    let mut trace_rows = Vec::with_capacity(history_rows.len().saturating_sub(skip));
    let mut report_rows = Vec::new();
    let mut git_control_candidate_events = 0usize;
    let mut payload_ready_events = 0usize;
    let mut payload_built_events = 0usize;
    let mut scoreable_payload_events = 0usize;
    let mut builder_rejected_events = 0usize;
    ...
    for (index, row) in history_rows.iter().enumerate().skip(skip) {
        let fingerprint = stable_real_traffic_fingerprint64(row.text.as_bytes());
        let event_id = format!(
            "codex_history_git_control_payload_dry_run::{}::{}::{}",
            row.session_id, row.ts, index
        );
        let request_fingerprint = format!("fnv1a64:{fingerprint:016x}");
        let exact_cache_key = Some(format!("codex_history_request:{fingerprint:016x}"));
        let mut nando_shadow_request = None;
        let mut notes = "not git_control route-gap candidate".to_owned();

        if route_catalog.classify_request_text(&row.text).is_none()
            && route_gap_family_key(&row.text) == REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY
        {
            git_control_candidate_events += 1;
            let readiness =
                analyze_route_gap_payload_readiness(REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY, &row.text);
            if readiness.payload_ready {
                payload_ready_events += 1;
                let built = build_git_control_dry_run_request(&event_id, &fingerprint, &row.text);
                match built {
                    Some(request) => {
                        let active_fringe_centers = request.active_fringe.len();
                        let slots = request.slots.len();
                        ...
                        *builder_status_counts
                            .entry(builder_status.clone())
                            .or_insert(0) += 1;
                        report_rows.push(RoleBindingGitControlPayloadDryRunRow {
                            event_id: event_id.clone(),
                            request_fingerprint: request_fingerprint.clone(),
                            route_key: REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY.to_owned(),
                            profile_id: REAL_TRAFFIC_GIT_CONTROL_PROFILE_ID.to_owned(),
                            readiness_payload_ready: true,
                            payload_built: true,
                            scoreable,
                            profile_registered,
                        ...
                        notes = format!(
                            "request-side git-control payload built; status={builder_status}; workspace mutations and verified accepts disabled"
                        );
                        nando_shadow_request = Some(request);
                    }
                    None => {
                ...
                *builder_status_counts
                    ...
                    .or_insert(0) += 1;
                notes = format!(
                    "git_control route-gap candidate rejected by readiness gate: {}",
                    readiness.missing_reasons.join(",")
                );
            }
        }
        ...
        trace_rows.push(RoleBindingRealTrafficTraceRow {
            schema_version: "nando_role_binding_real_traffic_trace_v1".to_owned(),
            trace_id: event_id,
            traffic_source: Some("codex_history_local_git_control_payload_dry_run".to_owned()),
            time_ms: Some(row.ts.saturating_mul(1000)),
            request_fingerprint: Some(request_fingerprint),
            response_fingerprint: None,
            tool_call_fingerprints: Vec::new(),
            verification_source: Some(
                "request-side git-control payload dry-run from local Codex prompt only; raw text, response text, target labels, proof labels, and workspace mutations are not used"
                    .to_owned(),
            ),
            llm_call: true,
            exact_cache_key,
    ...
    write_real_traffic_trace_jsonl(&trace_path, &trace_rows)?;
    let shadow_score_ready = profile_registered && scoreable_payload_events > 0;
    let report = RoleBindingGitControlPayloadDryRunReport {
        schema_version: "nando_role_binding_git_control_payload_dry_run_v1".to_owned(),
        verdict: if shadow_score_ready {
            "GIT_CONTROL_PAYLOAD_DRY_RUN_V1_REVIEW_SCOREABLE_PROFILE_READY"
        } else if scoreable_payload_events > 0 {
            "GIT_CONTROL_PAYLOAD_DRY_RUN_V1_REVIEW_SCOREABLE_PAYLOADS_PROFILE_MISSING"
        } else {
            "GIT_CONTROL_PAYLOAD_DRY_RUN_V1_REVIEW_NO_SCOREABLE_PAYLOADS"
        }
        .to_owned(),
        history_path: history_path.display().to_string(),
        registry_config_path: registry_config_path.display().to_string(),
        ...
        market_claim_allowed: false,
        rows: report_rows,
        claim_boundary: "Request-side dry-run payload builder only. It emits active_fringe/slots for git_control route-gap rows from prompt text only, writes no raw prompt text, performs no git command, keeps verified_safe_accept=None and expect_local_operator=false, and cannot prove savings.".to_owned(),
        next_engineering_debt: "Compile a git_control .nwrb scoring profile, rerun route-gap/feedback so the route leaves no-candidate, then attach git_status_and_command_outcome_verifier_v1 before any local accept or workspace mutation path.".to_owned(),
    };
    write_json_file(&report_path, &report)?;
    println!(
    ...
    println!(
        "  git_control_candidate_events: {}",
        report.git_control_candidate_events
    );
    println!("  payload_ready_events: {}", report.payload_ready_events);
    println!("  payload_built_events: {}", report.payload_built_events);
    println!(
    ...
    println!("  local_accepts_enabled: false");
    Err(
        "git-control payload dry-run is review-only; build profile+verifier before claims"
            .to_owned(),
    )
}

pub(crate) fn run_role_binding_real_traffic_git_control_profile_v1<I>(
    mut args: I,
) -> Result<(), String>
where
    I: Iterator<Item = String>,
{
    ...
    let package_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_GIT_CONTROL_PACKAGE_PATH));
    let registry_path = args
        .next()
        .map(PathBuf::from)
    ...
    let report_path = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_GIT_CONTROL_PROFILE_REPORT));

    let mut registry = read_json_file::<RoleBindingProfileRegistryConfig>(&base_registry_path)?;
    validate_registry_config(&registry)?;
    let trace_rows = read_real_traffic_trace_jsonl(&dry_run_trace_path)?;
    let build = build_git_control_role_binding_package_from_trace(&trace_rows)?;
    if let Some(parent) = package_path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            format!(
                "failed to create git-control package directory {}: {error}",
                parent.display()
            )
        })?;
    }
    fs::write(&package_path, &build.package_bytes).map_err(|error| {
        format!(
            "failed to write git-control package {}: {error}",
            package_path.display()
        )
    })?;
    let package_info =
        WavePredictorRoleBindingOffloadRuntime::inspect_package_bytes(&build.package_bytes)
            .map_err(|error| format!("failed to inspect git-control package: {error:?}"))?;
    let policy =
        WavePredictorRoleBindingOffloadPolicy::new(REAL_TRAFFIC_GIT_CONTROL_DISABLED_THRESHOLD)
            .map_err(|error| format!("invalid git-control disabled policy: {error:?}"))?;
    let sdk = WavePredictorRoleBindingOffloadRuntime::from_package_bytes_serving_packed_only(
        &build.package_bytes,
        policy,
    )
    .map_err(|error| format!("failed to load git-control package: {error:?}"))?;

    let requests = git_control_scoreable_requests(&trace_rows);
    let mut energy_margins = Vec::with_capacity(requests.len());
    let mut min_slot_margins = Vec::with_capacity(requests.len());
    let mut positive_margin_rows = 0usize;
    let mut strict_ordered_pass_rows = 0usize;
    ...
    for request in &requests {
        ...
        unexpected_local_accepts_under_disabled_threshold += usize::from(profile_accepts_score(
            ...
            energy_margin,
            first_slot_margin,
            REAL_TRAFFIC_GIT_CONTROL_DISABLED_THRESHOLD,
        ));
        energy_margins.push(energy_margin);
        min_slot_margins.push(min_slot_margin);
    }
    ...
    let profile = RoleBindingProfileConfig {
        ...
        profile_kind: "role_binding_nwrb".to_owned(),
        operator_classes: vec![
            "git_control".to_owned(),
            "workspace_command".to_owned(),
            "route_gap".to_owned(),
        ],
        package_path: package_path.clone(),
        runtime_bytes_estimate: sdk.bytes_estimate(),
        edge_count: package_info.edge_count,
        slot_count: 3,
        threshold: REAL_TRAFFIC_GIT_CONTROL_DISABLED_THRESHOLD,
        acceptance_policy: default_profile_acceptance_policy(),
        accepted_route_keys: vec![
            REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY.to_owned(),
            REAL_TRAFFIC_GIT_CONTROL_PROFILE_ID.to_owned(),
            "git_command_intent_payload_builder_v1".to_owned(),
        ],
    };
    registry
        .profiles
        .retain(|existing| existing.profile_id != profile.profile_id);
    registry.profiles.push(profile);
    registry.claim_boundary = "serving registry overlay for git-control .nwrb profile; generated from request-side dry-run payloads with threshold=i32::MAX so scoring telemetry is available but local accepts and workspace mutations remain disabled until command-outcome verification exists".to_owned();
    validate_registry_config(&registry)?;
    write_json_file(&registry_path, &registry)?;
    ...
    sorted_energy.sort_unstable();
    sorted_min_slot.sort_unstable();
    let report = RoleBindingGitControlProfileReport {
        schema_version: "nando_role_binding_git_control_profile_v1".to_owned(),
        verdict: if unexpected_local_accepts_under_disabled_threshold == 0
            && build.package_training_requests > 0
            && package_info.edge_count > 0
        {
            ...
            "GIT_CONTROL_PROFILE_V1_REVIEW_REPAIR_REQUIRED"
        }
        .to_owned(),
        base_registry_path: base_registry_path.display().to_string(),
        dry_run_trace_path: dry_run_trace_path.display().to_string(),
        ...
        local_accepts_enabled_on_real_traffic: false,
        market_claim_allowed: false,
        claim_boundary: "Git-control profile compilation only. The .nwrb package can measure real score/margins over request-side git intent payloads, but it cannot run git, local-accept, or prove savings without git_status_and_command_outcome_verifier_v1 and false_accepts=0.".to_owned(),
        next_engineering_debt: "Rerun route-gap/feedback with this registry so git_control leaves the no-candidate gap, then attach command outcome evidence before threshold calibration or any workspace mutation path.".to_owned(),
    };
    write_json_file(&report_path, &report)?;
    println!(
    ...
    I: Iterator<Item = String>,
{
    ...
    let read_inspect_dry_run = if read_inspect_dry_run_report_path.exists() {
        Some(read_json_file::<RoleBindingReadInspectPayloadDryRunReport>(
            &read_inspect_dry_run_report_path,
        )?)
    } else {
        None
    };
    let read_inspect_verification_audit = if read_inspect_verification_audit_report_path.exists() {
        Some(read_json_file::<RoleBindingVerificationHookAuditReport>(
    ...
    let read_inspect_local_accept_calibration_report_path =
        PathBuf::from(DEFAULT_READ_INSPECT_LOCAL_ACCEPT_CALIBRATION_REPORT);
    let read_inspect_local_accept_calibration =
        if read_inspect_local_accept_calibration_report_path.exists() {
            Some(
                read_json_file::<RoleBindingEditLocalAcceptCalibrationReport>(
                    &read_inspect_local_accept_calibration_report_path,
                )?,
            )
        } else {
            None
    ...
    if !forecast_has_planning_next_step
        && (planning_next_step_dry_run.is_some()
    ...
    if !forecast_has_read_inspect
        && (read_inspect_dry_run.is_some()
            || read_inspect_verification_audit.is_some()
            || read_inspect_local_accept_calibration.is_some())
    {
        let verification = verification_by_route
            .get(REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY)
            .copied();
        let payload_ready_events = read_inspect_dry_run
            .as_ref()
            .map(|report| report.payload_ready_events)
            .unwrap_or_default();
        let payload_built_events = read_inspect_dry_run
            .as_ref()
            .map(|report| report.payload_built_events)
            .unwrap_or_default();
        let scoreable_payload_events = verification
        ...
        let candidate_events = read_inspect_dry_run
            ...
            .unwrap_or_default();
        let local_accept_calibration_ran = read_inspect_local_accept_calibration.is_some();
        let local_accept_safe_policy_found = read_inspect_local_accept_calibration
            .as_ref()
            .map(|report| report.safe_policy_found)
            .unwrap_or(false);
        let local_accept_best_safe_true_accepts = read_inspect_local_accept_calibration
            .as_ref()
            .map(|report| report.best_safe_true_accepts)
            .unwrap_or_default();
        let local_accept_minimum_true_support = DEFAULT_REAL_TRAFFIC_MIN_SAFE_POLICY_TRUE_SUPPORT;
    ...
    if !forecast_has_git_control && git_control_dry_run.is_some() {
        let payload_ready_events = git_control_dry_run
            ...
            .map(|report| report.payload_ready_events)
            .unwrap_or_default();
        let payload_built_events = git_control_dry_run
            .as_ref()
            .map(|report| report.payload_built_events)
            .unwrap_or_default();
        ...
        let local_accept_best_safe_true_accepts = 0;
        let local_accept_minimum_true_support = DEFAULT_REAL_TRAFFIC_MIN_SAFE_POLICY_TRUE_SUPPORT;
        let local_accept_support_qualified = false;
        let stage = feedback_route_stage(FeedbackRouteStageInputs {
            payload_ready_events,
            payload_built_events,
            scoreable_payload_events,
        ...
        route_rows.push(RoleBindingFeedbackLoopRouteRow {
            ...
            candidate_events,
            non_exact_candidate_calls: candidate_events,
            payload_builder: "git_command_intent_payload_builder_v1".to_owned(),
            stage,
            next_action,
            payload_ready_events,
            payload_built_events,
    ...
    let report = RoleBindingFeedbackLoopReport {
        ...
        read_inspect_dry_run_report_path: read_inspect_dry_run
            .as_ref()
            .map(|_| read_inspect_dry_run_report_path.display().to_string()),
        read_inspect_local_accept_calibration_report_path:
            read_inspect_local_accept_calibration.as_ref().map(|_| {
                read_inspect_local_accept_calibration_report_path
                    .display()
                    .to_string()
            }),
        read_inspect_verification_audit_report_path: read_inspect_verification_audit
            .as_ref()
            .map(|_| {
                read_inspect_verification_audit_report_path
                    .display()
    ...
    if let Some(path) = &report.read_inspect_dry_run_report_path {
        println!("  read_inspect_dry_run_report: {path}");
    }
    if let Some(path) = &report.read_inspect_local_accept_calibration_report_path {
        println!("  read_inspect_local_accept_calibration_report: {path}");
    }
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingReadInspectPayloadDryRunReport {
    schema_version: String,
    verdict: String,
    history_path: String,
    registry_config_path: String,
    ...
    total_history_rows: usize,
    trace_rows_written: usize,
    read_inspect_candidate_events: usize,
    payload_ready_events: usize,
    payload_built_events: usize,
    scoreable_payload_events: usize,
    builder_rejected_events: usize,
    ...
    local_accepts_enabled: bool,
    market_claim_allowed: bool,
    rows: Vec<RoleBindingReadInspectPayloadDryRunRow>,
    claim_boundary: String,
    next_engineering_debt: String,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingReadInspectPayloadDryRunRow {
    event_id: String,
    request_fingerprint: String,
    route_key: String,
    profile_id: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingReadInspectProfileReport {
    schema_version: String,
    verdict: String,
    base_registry_path: String,
    dry_run_trace_path: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingGitControlPayloadDryRunReport {
    schema_version: String,
    verdict: String,
    history_path: String,
    registry_config_path: String,
    ...
    total_history_rows: usize,
    trace_rows_written: usize,
    git_control_candidate_events: usize,
    payload_ready_events: usize,
    payload_built_events: usize,
    scoreable_payload_events: usize,
    builder_rejected_events: usize,
    ...
    target_labels_used: bool,
    proof_labels_used: bool,
    workspace_mutation_enabled: bool,
    local_accepts_enabled: bool,
    market_claim_allowed: bool,
    rows: Vec<RoleBindingGitControlPayloadDryRunRow>,
    claim_boundary: String,
    next_engineering_debt: String,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingGitControlPayloadDryRunRow {
    event_id: String,
    request_fingerprint: String,
    route_key: String,
    profile_id: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct RoleBindingGitControlProfileReport {
    schema_version: String,
    verdict: String,
    base_registry_path: String,
    dry_run_trace_path: String,
...
fn build_read_inspect_role_binding_package_from_trace(
    trace_rows: &[RoleBindingRealTrafficTraceRow],
) -> Result<ReadInspectPackageBuild, String> {
    let config = WavePredictorHebbianConfig {
        state_delta_binding_action_base: Some(REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_BASE),
        state_delta_binding_action_count: REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE,
        state_delta_binding_role_base: Some(REAL_TRAFFIC_READ_INSPECT_ROLE_BASE),
        state_delta_binding_role_stride: REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE,
        state_delta_binding_role_count: 4,
        state_delta_binding_slot_scoped_action_page_bits: 12,
        state_delta_binding_slot_scoped_action_page_mask: 1_u64
            << (REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_BASE >> 12),
        state_delta_binding_slot_scoped_action_source_bits:
            REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_SHIFT as u8,
        weight_limit: 2_048,
        ..WavePredictorHebbianConfig::default()
    };
    let role_end = REAL_TRAFFIC_READ_INSPECT_ROLE_BASE
        + REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE * u32::from(config.state_delta_binding_role_count);
    let action_end =
        REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_BASE + REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE;
    let center_count = role_end.max(action_end) as usize;
    let mut field = WavePredictorHebbianField::new(center_count, config);
    let mut package_training_requests = 0usize;
    let mut positive_updates = 0usize;
    ...
    for request in read_inspect_scoreable_requests(trace_rows) {
        let active_fringe = request
            .active_fringe
            .iter()
            .map(|active| WavePredictorActiveCenter {
    ...
    if package_training_requests == 0 {
        return Err("read-inspect package builder found no scoreable dry-run requests".to_owned());
    }
    if changed_edges == 0 {
        return Err("read-inspect package builder produced no role-binding edges".to_owned());
    }
    let package_bytes = field
        .compile_flat_role_binding_table()
        .to_bytes()
        .map_err(|error| format!("failed to serialize read-inspect .nwrb package: {error:?}"))?;
    Ok(ReadInspectPackageBuild {
        package_bytes,
        package_training_requests,
        positive_updates,
        negative_updates,
...
fn read_inspect_scoreable_requests(
    trace_rows: &[RoleBindingRealTrafficTraceRow],
) -> Vec<RoleBindingProfileScoreRequest> {
    trace_rows
        .iter()
...
fn build_git_control_role_binding_package_from_trace(
    trace_rows: &[RoleBindingRealTrafficTraceRow],
) -> Result<GitControlPackageBuild, String> {
    let config = WavePredictorHebbianConfig {
        state_delta_binding_action_base: Some(REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_BASE),
        state_delta_binding_action_count: REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE,
        state_delta_binding_role_base: Some(REAL_TRAFFIC_GIT_CONTROL_ROLE_BASE),
        state_delta_binding_role_stride: REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE,
        state_delta_binding_role_count: 4,
        state_delta_binding_slot_scoped_action_page_bits: 12,
        state_delta_binding_slot_scoped_action_page_mask: 1_u64
            << (REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_BASE >> 12),
        state_delta_binding_slot_scoped_action_source_bits:
            REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_SHIFT as u8,
        weight_limit: 2_048,
        ..WavePredictorHebbianConfig::default()
    };
    let role_end = REAL_TRAFFIC_GIT_CONTROL_ROLE_BASE
        + REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE * u32::from(config.state_delta_binding_role_count);
    let action_end =
        REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_BASE + REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE;
    let center_count = role_end.max(action_end) as usize;
    let mut field = WavePredictorHebbianField::new(center_count, config);
    let mut package_training_requests = 0usize;
    let mut positive_updates = 0usize;
    ...
    for request in git_control_scoreable_requests(trace_rows) {
        let active_fringe = request
            .active_fringe
            .iter()
            .map(|active| WavePredictorActiveCenter {
    ...
    if package_training_requests == 0 {
        return Err("git-control package builder found no scoreable dry-run requests".to_owned());
    }
    if changed_edges == 0 {
        return Err("git-control package builder produced no role-binding edges".to_owned());
    }
    let package_bytes = field
        .compile_flat_role_binding_table()
        .to_bytes()
        .map_err(|error| format!("failed to serialize git-control .nwrb package: {error:?}"))?;
    Ok(GitControlPackageBuild {
        package_bytes,
        package_training_requests,
        positive_updates,
        negative_updates,
...
fn git_control_scoreable_requests(
    trace_rows: &[RoleBindingRealTrafficTraceRow],
) -> Vec<RoleBindingProfileScoreRequest> {
    trace_rows
        .iter()
...
fn build_read_inspect_dry_run_request(
    ...
    let tokens = extract_read_inspect_tokens(text)?;
    let mut active_fringe = Vec::new();
    active_fringe.extend(read_inspect_operator_centers());
    active_fringe.extend(read_inspect_role_surface_centers(
        REAL_TRAFFIC_READ_INSPECT_PATH_ROLE_SLOT,
        &tokens.path_token,
    ));
    active_fringe.extend(read_inspect_role_surface_centers(
        REAL_TRAFFIC_READ_INSPECT_REQUEST_ROLE_SLOT,
        &tokens.request_token,
    ));
    active_fringe.extend(read_inspect_role_surface_centers(
        REAL_TRAFFIC_READ_INSPECT_EVIDENCE_ROLE_SLOT,
        &tokens.evidence_token,
    ));
    active_fringe.extend(read_inspect_role_surface_centers(
        REAL_TRAFFIC_READ_INSPECT_CHECK_ROLE_SLOT,
        &tokens.check_token,
    ));
    let active_fringe = merge_profile_active_centers(active_fringe);

    let mut slots = Vec::new();
    if let Some(slot) = read_inspect_request_score_slot(
        0,
        &tokens.path_token,
        REAL_TRAFFIC_READ_INSPECT_WRONG_TOKEN,
    ) {
        slots.push(slot);
    }
    if let Some(slot) = read_inspect_request_score_slot(
    ...
    Some(RoleBindingProfileScoreRequest {
        request_id: event_id.to_owned(),
        route_key: Some(REAL_TRAFFIC_READ_INSPECT_ROUTE_KEY.to_owned()),
        profile_id: Some(REAL_TRAFFIC_READ_INSPECT_PROFILE_ID.to_owned()),
        exact_cache_key: Some(format!("codex_history_request:{fingerprint:016x}")),
        active_fringe,
        slots,
        // Dry-run only: read-only file/path verification is not attached yet,
        // so local accept must remain disabled.
        expect_local_operator: Some(false),
    })
}

fn read_inspect_operator_centers() -> Vec<RoleBindingProfileActiveCenterRow> {
    [
        (0, REAL_TRAFFIC_READ_INSPECT_PATH_ROLE_SLOT),
        (1, REAL_TRAFFIC_READ_INSPECT_REQUEST_ROLE_SLOT),
        (2, REAL_TRAFFIC_READ_INSPECT_EVIDENCE_ROLE_SLOT),
        (2, REAL_TRAFFIC_READ_INSPECT_CHECK_ROLE_SLOT),
    ]
    .into_iter()
    .map(
        |(output_slot, role_slot)| RoleBindingProfileActiveCenterRow {
            center_id: REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_BASE
                + read_inspect_operator_pair_lane(output_slot, role_slot),
            strength: 8,
        },
    )
    .collect()
...
fn read_inspect_operator_pair_lane(output_slot: u8, role_slot: u8) -> u32 {
    (u32::from(output_slot) << REAL_TRAFFIC_READ_INSPECT_OPERATOR_PAIR_SHIFT) | u32::from(role_slot)
}

fn read_inspect_role_surface_centers(
    role_slot: u8,
    token: &str,
) -> Vec<RoleBindingProfileActiveCenterRow> {
    let slot_base = REAL_TRAFFIC_READ_INSPECT_ROLE_BASE
        + u32::from(role_slot).saturating_mul(REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE);
    surface_lane_centers_folded_for_profile(
        token,
        slot_base,
        REAL_TRAFFIC_READ_INSPECT_PAGE_SIZE,
        REAL_TRAFFIC_READ_INSPECT_TOP_ROLE_L1_LANES,
    )
}

fn read_inspect_request_score_slot(
    binding_output_slot: u8,
    correct_token: &str,
    wrong_token: &str,
) -> Option<RoleBindingProfileScoreSlotRow> {
    ...
    let positive_impulses = discriminative_profile_impulses(
        ...
        target_wave.lanes(),
        wrong_wave.lanes(),
        REAL_TRAFFIC_READ_INSPECT_STATE_DELTA_LANES_PER_SIDE,
    );
    let negative_impulses = discriminative_profile_impulses(
        base_wave.lanes(),
        wrong_wave.lanes(),
...
fn extract_read_inspect_tokens(text: &str) -> Option<ReadInspectTokens> {
    let tokens = extract_request_side_edit_tokens(text, 24);
    if tokens.is_empty() {
        return None;
    }
    let lower = text.to_lowercase();
    let request_token = first_matching_branch_token(
        &lower,
        &[
            "читай",
            "прочитай",
    ...
    let path_token = quoted_or_marked_chunks(text)
        .into_iter()
        .find_map(|chunk| {
            extract_request_side_edit_tokens(&chunk, 1)
                .into_iter()
                .find(|token| looks_like_read_inspect_artifact_token(token))
        })
        .or_else(|| {
            tokens
                .iter()
        ...
        .or_else(|| {
            tokens
                .iter()
                .find(|token| token.as_str() != request_token)
                .cloned()
        })?;
    let evidence_token = tokens
        ...
        .cloned()
        .unwrap_or_else(|| path_token.clone());
    let check_token = first_matching_branch_token(
        &lower,
        &[
            "проверь",
            "verify",
    ...
    .unwrap_or_else(|| evidence_token.clone());
    Some(ReadInspectTokens {
        path_token,
        request_token,
        evidence_token,
        check_token,
...
fn looks_like_read_inspect_artifact_token(token: &str) -> bool {
    token.contains('/')
        || contains_any(
            token,
            &[
                ".rs", ".py", ".md", ".json", ".jsonl", ".toml", ".yaml", ".yml", ".txt", ".log",
            ],
        )
        || contains_any(
            token,
            &[
                "docs", "crates", "target", "report", "notes", "readme", "memory", "history",
            ],
        )
}

#[derive(Clone, Debug)]
struct ReadInspectTokens {
    path_token: String,
...
fn build_git_control_dry_run_request(
    event_id: &str,
    fingerprint: &u64,
    text: &str,
) -> Option<RoleBindingProfileScoreRequest> {
    let tokens = extract_git_control_tokens(text)?;
    let mut active_fringe = Vec::new();
    active_fringe.extend(git_control_operator_centers());
    active_fringe.extend(git_control_role_surface_centers(
        REAL_TRAFFIC_GIT_CONTROL_INTENT_ROLE_SLOT,
        &tokens.intent_token,
    ));
    active_fringe.extend(git_control_role_surface_centers(
        REAL_TRAFFIC_GIT_CONTROL_COMMAND_ROLE_SLOT,
        &tokens.command_token,
    ));
    active_fringe.extend(git_control_role_surface_centers(
        REAL_TRAFFIC_GIT_CONTROL_TARGET_ROLE_SLOT,
        &tokens.target_token,
    ));
    active_fringe.extend(git_control_role_surface_centers(
        REAL_TRAFFIC_GIT_CONTROL_SAFETY_ROLE_SLOT,
        &tokens.safety_token,
    ));
    let active_fringe = merge_profile_active_centers(active_fringe);

    let mut slots = Vec::new();
    if let Some(slot) = git_control_request_score_slot(
        0,
        &tokens.command_token,
        REAL_TRAFFIC_GIT_CONTROL_WRONG_TOKEN,
    ) {
        slots.push(slot);
    }
    if let Some(slot) = git_control_request_score_slot(
    ...
    Some(RoleBindingProfileScoreRequest {
        request_id: event_id.to_owned(),
        route_key: Some(REAL_TRAFFIC_GIT_CONTROL_ROUTE_KEY.to_owned()),
        profile_id: Some(REAL_TRAFFIC_GIT_CONTROL_PROFILE_ID.to_owned()),
        exact_cache_key: Some(format!("codex_history_request:{fingerprint:016x}")),
        active_fringe,
        slots,
        // Dry-run only: command execution/outcome verification is not attached
        // yet, so local accept and workspace mutation must remain disabled.
        expect_local_operator: Some(false),
    })
}

fn git_control_operator_centers() -> Vec<RoleBindingProfileActiveCenterRow> {
    [
        (0, REAL_TRAFFIC_GIT_CONTROL_COMMAND_ROLE_SLOT),
        (1, REAL_TRAFFIC_GIT_CONTROL_TARGET_ROLE_SLOT),
        (2, REAL_TRAFFIC_GIT_CONTROL_SAFETY_ROLE_SLOT),
        (2, REAL_TRAFFIC_GIT_CONTROL_INTENT_ROLE_SLOT),
    ]
    .into_iter()
    .map(
        |(output_slot, role_slot)| RoleBindingProfileActiveCenterRow {
            center_id: REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_BASE
                + git_control_operator_pair_lane(output_slot, role_slot),
            strength: 8,
        },
    )
    .collect()
...
fn git_control_operator_pair_lane(output_slot: u8, role_slot: u8) -> u32 {
    (u32::from(output_slot) << REAL_TRAFFIC_GIT_CONTROL_OPERATOR_PAIR_SHIFT) | u32::from(role_slot)
}

fn git_control_role_surface_centers(
    role_slot: u8,
    token: &str,
) -> Vec<RoleBindingProfileActiveCenterRow> {
    let slot_base = REAL_TRAFFIC_GIT_CONTROL_ROLE_BASE
        + u32::from(role_slot).saturating_mul(REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE);
    surface_lane_centers_folded_for_profile(
        token,
        slot_base,
        REAL_TRAFFIC_GIT_CONTROL_PAGE_SIZE,
        REAL_TRAFFIC_GIT_CONTROL_TOP_ROLE_L1_LANES,
    )
}

fn git_control_request_score_slot(
    binding_output_slot: u8,
    correct_token: &str,
    wrong_token: &str,
) -> Option<RoleBindingProfileScoreSlotRow> {
    ...
    let positive_impulses = discriminative_profile_impulses(
        ...
        target_wave.lanes(),
        wrong_wave.lanes(),
        REAL_TRAFFIC_GIT_CONTROL_STATE_DELTA_LANES_PER_SIDE,
    );
    let negative_impulses = discriminative_profile_impulses(
        base_wave.lanes(),
        wrong_wave.lanes(),
...
fn extract_git_control_tokens(text: &str) -> Option<GitControlTokens> {
    let tokens = extract_request_side_edit_tokens(text, 24);
    if tokens.is_empty() {
        return None;
    }
    let lower = text.to_lowercase();
    let intent_token = first_matching_branch_token(
        &lower,
        &[
            "git",
            "коммит",
    ...
    .unwrap_or_else(|| tokens[0].clone());
    let command_token = first_matching_branch_token(
        &lower,
        &[
            "commit",
            "коммит",
    ...
    .unwrap_or_else(|| intent_token.clone());
    let target_token = quoted_or_marked_chunks(text)
        .into_iter()
        .find_map(|chunk| {
            extract_request_side_edit_tokens(&chunk, 1)
                .into_iter()
                .find(|token| looks_like_git_control_target_token(token))
        })
        .or_else(|| {
            tokens
                .iter()
        ...
        .or_else(|| {
            tokens
                .iter()
                .find(|token| token.as_str() != intent_token && token.as_str() != command_token)
                .cloned()
        })
        .unwrap_or_else(|| command_token.clone());
    let safety_token = first_matching_branch_token(
        &lower,
        &[
            "status",
            "diff",
...
fn looks_like_git_control_target_token(token: &str) -> bool {
    token.contains('/')
        || contains_any(
            token,
            &[
...
fn cpu_route_builder_recommendation(route_key: &str) -> (String, String) {
    ...
    } else if route_key == REAL_TRAFFIC_PLANNING_ROUTE_KEY {
        (
