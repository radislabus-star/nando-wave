...
use super::online_miner_targeted_aggregate_provider_export_admission::run_phase_stream_online_miner_targeted_aggregate_provider_export_admission_v1;
...
const DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_REPORT: &str = "target/nando-wave/streaming/phase-stream-online-miner-targeted-aggregate-provider-export-autoscan-v1.report.json";
const DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_SCAN_DIR: &str = "target/nando-wave/streaming";
const DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_WORK_DIR: &str =
    "target/nando-wave/streaming/targeted-aggregate-provider-export-autoscan-v1";
...
pub(crate) fn run_phase_stream_online_miner_targeted_aggregate_provider_export_autoscan_v1<I>(
    mut args: I,
) -> Result<(), String>
where
    I: Iterator<Item = String>,
{
    let report_path = args.next().map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_REPORT)
    });
    let scan_dir = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_SCAN_DIR));
    let work_dir = args.next().map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(DEFAULT_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_WORK_DIR)
    });
    ...
    std::fs::create_dir_all(&work_dir).map_err(|error| {
        format!(
            "failed to create targeted aggregate provider export autoscan work dir '{}': {error}",
            work_dir.display()
        )
    })?;
    ...
    for (index, candidate) in candidates.iter().take(max_evaluated_candidates).enumerate() {
        ...
        evaluated.push(EvaluatedProviderExportCandidate {
            ...
            local_accept_enabled: json_bool(&report, &["local_accept_enabled"]).unwrap_or(true),
            billing_request_rows: json_usize_path(
                &report,
                &["billing_request", "billing_request_rows"],
            )
            .unwrap_or(0),
            total_tokens_requiring_billing: json_usize_path(
                &report,
                &["billing_request", "total_tokens_requiring_billing"],
            )
            .unwrap_or(0),
            rows_enriched_provider_cost: json_usize_path(
                &report,
                &["billing_evidence_gate", "rows_enriched_provider_cost"],
            )
            .unwrap_or(0),
            rows_enriched_provider_tokens: json_usize_path(
                &report,
                &["billing_evidence_gate", "rows_enriched_provider_tokens"],
            )
            .unwrap_or(0),
            missing_billing_request_rows: json_usize_path(
                &report,
                &["billing_evidence_gate", "missing_billing_request_rows"],
            )
            .unwrap_or(0),
            blocker: json_string(&report, &["blocker"]).unwrap_or_default(),
        });
    }
    ...
    let money_ready_count = evaluated
        ...
        .count();
    let local_accept_enabled_count = evaluated
        .iter()
        .filter(|candidate| candidate.local_accept_enabled)
        .count();
    let verdict = if product_ready_count > 0 && money_ready_count > 0 {
        "PHASE_STREAM_ONLINE_MINER_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_V1_FOUND_MONEY_READY_EVIDENCE"
    } else if shadow_ready_count > 0 || matching_candidate_count > 0 {
        "PHASE_STREAM_ONLINE_MINER_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_V1_NO_MONEY_READY_EVIDENCE"
    } else {
        "PHASE_STREAM_ONLINE_MINER_TARGETED_AGGREGATE_PROVIDER_EXPORT_AUTOSCAN_V1_NO_MATCHING_CANDIDATE"
    };
    ...
    insert_json(
        &mut report,
        "report_kind",
        "phase_stream_online_miner_targeted_aggregate_provider_export_autoscan_v1",
    )?;
    insert_json(
        &mut report,
        "mode",
    ...
    insert_json(&mut report, "aggregate_report_path", aggregate_report_path)?;
    insert_json(
        &mut report,
    ...
    insert_json(&mut report, "shadow_ready_count", shadow_ready_count)?;
    insert_json(
        &mut report,
    ...
    insert_json(
        &mut report,
        "attested_candidate_count",
        attested_candidate_count,
    )?;
    insert_json(
        &mut report,
        "provider_gate_product_ready_count",
        provider_gate_product_ready_count,
    )?;
    insert_json(
        &mut report,
        "provider_gate_money_ready_count",
        provider_gate_money_ready_count,
    )?;
    ...
    insert_json(
        &mut report,
        "boundary",
        "targeted aggregate provider export autoscan only: scans local candidate export files for aggregate billing keys and runs provider-export admission on matching candidates; does not compile, mine, serve, mutate registry, enable local_accept, estimate missing money, or use legacy nwrb",
    )?;
    let report = Value::Object(report);
    write_json_file(&report_path, &report)?;
    println!("phase_stream_online_miner_targeted_aggregate_provider_export_autoscan_v1:");
    ...
    println!("  shadow_ready_count: {shadow_ready_count}");
...
fn count_selected_key_hits(
    ...
    let selected_keys = selected_keys.iter().cloned().collect::<BTreeSet<_>>();
    if path
        .extension()
        .and_then(|value| value.to_str())
        .is_some_and(|extension| matches!(extension, "csv" | "tsv"))
    {
        return count_selected_key_hits_delimited(path, &selected_keys);
    }
    ...
    for (line_index, line) in reader.lines().enumerate() {
        ...
        if let Ok(row) = serde_json::from_str::<Value>(trimmed) {
            ...
            for key in split_match_key_text(trimmed) {
                if selected_keys.contains(&key) {
                    matched_keys.insert(key);
                }
            }
    ...
    Ok((matched_keys.len(), line_count))
}

fn count_selected_key_hits_delimited(
    path: &Path,
    ...
    for (line_index, line) in lines {
        ...
        for (header, value) in headers.iter().zip(values) {
            for key in candidate_match_keys_from_field(header, &value) {
                if selected_keys.contains(&key) {
                    matched_keys.insert(key);
                }
            }
...
fn candidate_match_keys(row: &Value) -> Vec<String> {
    let mut keys = Vec::new();
    push_string_array(&mut keys, row, &["match_keys"]);
    push_string_array(&mut keys, row, &["external_provider_correlation_keys"]);
    push_string_array(
        &mut keys,
        row,
...
fn candidate_match_keys_from_field(header: &str, value: &str) -> Vec<String> {
    ...
    match header {
        "request_fingerprint" | "custom_id" => {
            vec![value.to_owned(), format!("request_fingerprint:{value}")]
        }
        "exact_cache_key" | "cache_key" => {
            vec![value.to_owned(), format!("exact_cache_key:{value}")]
        }
        "match_keys" | "external_provider_correlation_keys" => split_match_key_text(value),
        _ => Vec::new(),
    }
}

fn push_string_array(keys: &mut Vec<String>, row: &Value, path: &[&str]) {
    let mut current = row;
    for key in path {
...
fn push_match_key_paths(keys: &mut Vec<String>, row: &Value, label: &str, paths: &[&[&str]]) {
    for path in paths {
...
fn split_match_key_text(value: &str) -> Vec<String> {
    value
