...
run "modadd goal smoke" bash -c 'cargo run -q -p nando-cli -- organ128-modadd-eval 7 31 64 64 | tee /tmp/nando-modadd-goal.log && rg -q "mode_status: (organ128_modadd_key_mode_ablation_passed|organ128_modadd_candidate|not_found_organ128_modadd)" /tmp/nando-modadd-goal.log && rg -q "label_shuffle_accuracy:" /tmp/nando-modadd-goal.log && rg -q "no_shortcut_control:" /tmp/nando-modadd-goal.log'
